  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case "tool:start": {
        ...
        // — but only when it's a short introductory utterance (<=160 chars).
        // Once real answer content has been emitted (long text after a tool
        // finished), we do NOT fold it back — that causes a visible "jump"
        // between thinking and body output (用户反馈的"反复横跳").
