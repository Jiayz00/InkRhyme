...
export const STREAM_TEXT_FLUSH_MS = 64; // ~15 fps, 足够自然且给 Markdown 解析留时间
  ...
  for (const delta of deltas) {
    ...
    if (delta.kind === "thinking") {
      ...
      if (last?.type === "thinking") {
        ...
        // thinking 先到（或 thinking_start 丢失）也不能丢内容：创建一个新 thinking part，
        // 标记为 streaming 让 Reasoning 折叠框保持"进行中"样式，后续 thinking_end 会关掉。
        next.push({ type: "thinking", content: delta.text, streaming: true });
      }
      continue;
    }
...
export function attachSessionStreamListeners({
  ...
  const textDeltaBatcher = createStreamTextDeltaBatcher((deltas) => {
    set((state) => {
      ...
      const [messages, stream] = getOrCreateStream(session.messages, streamTs);
      const prevParts = stream.parts ?? [];
      const parts = applyStreamTextDeltas(prevParts, deltas);
      // applyStreamTextDeltas 返回的数组和内容都没变时（空 deltas/都是空文本），
      // 跳过 setState 避免一帧一次的 re-render。
      if (parts.length === prevParts.length && parts.every((p, i) => p === prevParts[i])) {
        return state;
      }
      ...
      set((state) => ({
        sessions: updateSession(state.sessions, sessionId, (runtime) => {
          ...
          if (data.tool === "sub_agent") {
            const last = parts[parts.length - 1];
            // 把正文回塞进 thinking 的启发式：只有在"还没有任何真正的 thinking 块
            // 出现过 && 这段 text 很短（<= 160 个可打印字符，更像一句过渡语而不是
            // 完整回答）"时才回塞。用户已经看到过展开的回答正文、或模型先输出过
            // 一大段内容时，直接保留它的 text 身份，避免"说话前的一句话突然被
            // 折叠到思考里"的横跳。
            const SAFE_TEXT_CHARS = 160;
            const hasAnyThinking = parts.some(p => p.type === "thinking");
            if (
              last?.type === "text" && last.content
              && !hasAnyThinking
              && [...last.content].length <= SAFE_TEXT_CHARS
            ) {
              parts.pop();
