...
import {
  loadStudioGlobalConfig,
  saveStudioGlobalConfig,
  type StudioGlobalConfig,
} from "../utils/studio-global-config.js";
...
function projectPackageMetaPath(root: string, id: string, meta: string): string {
  return join(projectSkillDir(root, id), meta);
}
...
function toStudioSkill(skill: AgentSkill, root: string, projectSkillIds: ReadonlySet<string>): StudioSkill {
  ...
  return {
    ...
    body: skill.body,
    source: isProjectFile ? "project" : (skill.source ?? "external"),
...
function toStudioPackage(pkg: SkillPackage, root: string, projectPackageIds: ReadonlySet<string>): StudioSkillPackage {
  const pkgDir = projectSkillDir(root, pkg.id);
  const isProject = projectPackageIds.has(pkg.id);
  return {
    id: pkg.id,
    name: pkg.name,
    description: pkg.description,
    source: isProject ? "project" : (pkg.source ?? "external"),
    editable: isProject,
    skillIds: pkg.skillIds,
    path: isProject ? relative(root, pkgDir) : undefined,
    tags: pkg.tags ?? [],
  };
}
...
type SkillImportClassification =
  | { readonly kind: "standalone"; readonly files: ReadonlyArray<StudioSkillImportFile>; readonly manifestPath: string }
  | { readonly kind: "package"; readonly files: ReadonlyArray<StudioSkillImportFile>; readonly subManifests: readonly { readonly relativeDir: string; readonly manifestPath: string }[]; readonly packageMeta?: StudioSkillImportFile };
...
function isPackageMetaFile(path: string): boolean {
  const base = path.includes("/") ? path.slice(path.lastIndexOf("/") + 1) : path;
  return base === "PACKAGE.yaml" || base === "PACKAGE.yml" || base === "skill-package.yaml" || base === "skill-package.yml";
}

function normalizeSkillImportFiles(value: unknown): SkillImportClassification {
  ...
  if (manifests.length === 0) {
    throw new ApiError(400, "INVALID_SKILL_IMPORT", "Skill import must contain at least one SKILL.md");
  }

  // 情况 A：顶层有 SKILL.md → 单 skill 导入（standalone 结构）
  if (manifests.some((file) => file.path === "SKILL.md")) {
    const manifestPath = "SKILL.md";
    return { kind: "standalone", files, manifestPath };
  }

  // 情况 B：没有顶层 SKILL.md，但有多个 SKILL.md → Skill 包；可以带一个顶层 PACKAGE yaml
  if (manifests.length >= 2) {
    const packageMeta = files.find((file) => isPackageMetaFile(file.path) && !file.path.includes("/"));
    const subMap = new Map<string, StudioSkillImportFile>();
    for (const mf of manifests) {
      const folder = mf.path.slice(0, -"/SKILL.md".length);
      if (folder.includes("/")) {
        // 包导入时要求直接一级子目录，不允许更深，否则存储到 .agents/skills/<pkgId>/<nested-path>/ 太深
        throw new ApiError(400, "INVALID_SKILL_IMPORT", "Skill package SKILL.md files must be direct child folders");
      }
      subMap.set(folder, mf);
    }
    const subManifests = [...subMap.entries()].map(([relativeDir, manifest]) => ({ relativeDir, manifestPath: manifest.path })).sort((a, b) => a.relativeDir.localeCompare(b.relativeDir));
    // 检查所有文件都在某个子 skill 目录或 package 顶层
    for (const file of files) {
      if (isPackageMetaFile(file.path) && !file.path.includes("/")) continue;
      if (file.path.startsWith(".") && file.path.includes("/")) continue;
      if (!file.path.includes("/")) {
        // 顶层非 package meta 文件：可接受（README / LICENSE 等参考资料），但不归类
        continue;
      }
      const folder = file.path.slice(0, file.path.indexOf("/"));
      if (!subMap.has(folder)) {
        throw new ApiError(400, "INVALID_SKILL_IMPORT_PATH", `File "${file.path}" doesn't belong to any package sub-skill folder`);
      }
    }
    return { kind: "package", files, subManifests, packageMeta };
  }

  // 情况 C：只有 1 个 SKILL.md，但它在子目录里（web folder 上传时带了父文件夹名）
  // 这等同于 standalone：把这个子目录作为 root
  const manifest = manifests[0]!;
  const folder = manifest.path.slice(0, -"/SKILL.md".length);
  for (const file of files) {
  ...
  return { kind: "standalone", files, manifestPath: manifest.path };
}

type SkillImportResult =
  | { readonly kind: "standalone"; readonly skill: AgentSkill }
  | { readonly kind: "package"; readonly packageId: string; readonly skillIds: readonly string[] };
...
async function importStudioSkillFolder(
  ...
  const classification = normalizeSkillImportFiles(record.files);
  ...
  if (classification.kind === "standalone") {
    const { files, manifestPath } = classification;
    ...
    return { kind: "standalone", skill: { ...parsed, source: "project", baseDir: targetDir } };
  }

  // Package 导入
  const { files, subManifests, packageMeta } = classification;
  // 先 parse 所有 sub skill，得到 ids；再得到 packageId（按 manifest 命名）
  const parsedSkills: Array<{ dir: string; skill: AgentSkill }> = [];
  const packageDiagnostics: string[] = [];
  for (const { relativeDir, manifestPath } of subManifests) {
    const manifest = files.find((file) => file.path === manifestPath);
    if (!manifest) continue;
    try {
      const parsed = parseAgentSkillDocument(manifest.buffer.toString("utf-8"), {
        skillPath: join(root, relativeDir, "SKILL.md"),
        source: "project",
      });
      parsedSkills.push({ dir: relativeDir, skill: parsed });
    } catch (error) {
      packageDiagnostics.push(`${relativeDir}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  if (parsedSkills.length === 0) {
    throw new ApiError(400, "INVALID_SKILL_PACKAGE", `Skill package has no valid sub-skill manifests: ${packageDiagnostics.join("; ") || "unknown"}`);
  }

  // 计算 packageId：优先从 packageMeta 读 name；否则退化为各 skill 目录的共同父/用户上传最顶层文件夹名（这里用第一个子目录之前的 common prefix，但简单化就用上传的"顶层有没有包 meta 名"；都没有就用时间戳 + sorted ids 的 digest。）
  let packageName: string | undefined;
  let packageDescription: string | undefined;
  let packageTags: string[] | undefined;
  if (packageMeta) {
    try {
      const meta = yaml.load(packageMeta.buffer.toString("utf-8")) as unknown;
      if (meta && typeof meta === "object" && !Array.isArray(meta)) {
        const rec = meta as Record<string, unknown>;
        if (typeof rec.name === "string" && rec.name.trim()) packageName = rec.name.trim();
        if (typeof rec.description === "string" && rec.description.trim()) packageDescription = rec.description.trim();
        if (Array.isArray(rec.tags) && rec.tags.every((t) => typeof t === "string")) packageTags = rec.tags;
      }
    } catch {
      /* ignore malformed package yaml — fall back to default */
    }
  }

  // 确定一个稳定的 packageId：优先根据 packageName；否则用前几个 skill 的 id join 短 hash
  const joinedForHash = parsedSkills.map((s) => s.skill.id).sort().join(",");
  const packageFallback = `pkg-${hashStringShort(joinedForHash)}`;
  const packageId: string = (() => {
    const base = packageName || packageFallback;
    const normalized = base
      .normalize("NFKD")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "");
    return (/^[a-z]/.test(normalized) ? normalized : normalized ? `pkg-${normalized}` : packageFallback).slice(0, 64);
  })();

  const targetDir = projectSkillDir(root, packageId);
  try {
    await access(targetDir);
    throw new ApiError(409, "SKILL_EXISTS", `Project skill package already exists: ${packageId}`);
  } catch (error) {
    if (error instanceof ApiError) throw error;
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
  }

  const finalSkills = parsedSkills.map(({ dir, skill }) => ({
    dir,
    id: skill.id,
    originalDir: dir,
  }));

  const finalPackageName = packageName || `${packageId} skill package`;
  const finalPackageDescription = packageDescription || `Skill package（${finalSkills.length} 个 skill，分别是：${finalSkills.map((s) => s.id).join(" / ")}）`;
  const finalTags = packageTags ?? [];

  try {
    await mkdir(stagingDir, { recursive: true });
    for (const file of files) {
      // package meta 文件直接放根（保持原文件名）
      if (packageMeta && file.path === packageMeta.path) {
        const base = file.path.includes("/") ? file.path.slice(file.path.lastIndexOf("/") + 1) : file.path;
        await writeFile(join(stagingDir, base), file.buffer);
        continue;
      }
      if (!file.path.includes("/")) continue; // 顶层参考文件（README 等）保留（相对原路径放到根）
      const slash = file.path.indexOf("/");
      const folder = file.path.slice(0, slash);
      const rest = file.path.slice(slash + 1);
      // 子 skill 的目录名保持与上传时一致（用户习惯用什么就保留什么；load 时会按 SKILL.md 生成 skill id）
      const destination = join(stagingDir, folder, rest);
      await mkdir(dirname(destination), { recursive: true });
      await writeFile(destination, file.buffer);
    }
    // 如果用户没有提供 package meta 文件，写一个默认的 PACKAGE.yaml，方便 disk 上看
    if (!packageMeta) {
      const defaultMeta = {
        name: finalPackageName,
        description: finalPackageDescription,
        tags: finalTags,
      };
      await writeFile(join(stagingDir, "PACKAGE.yaml"), yaml.dump(defaultMeta, { lineWidth: 120 }));
    }
    await rename(stagingDir, targetDir);
  } catch (error) {
    await rm(stagingDir, { recursive: true, force: true });
  ...
  return { kind: "package", packageId, skillIds: finalSkills.map((s) => s.id) };
}
...
function hashStringShort(input: string): string {
  ...
  h1 ^= h1 >>> 13;
  return (h1 >>> 0).toString(36).slice(0, 6);
}

async function loadStudioSkills(root: string) {
  const configured = await loadConfiguredAgentSkills({ projectRoot: root });
  const { skillIds: projectSkillIds, packageIds: projectPackageIds } = await listProjectSkillAndPackageIds(root);
  const registry = createSkillRegistry({ skills: configured.skills, packages: configured.packages });
  return {
    skills: registry.listSkills().map((skill) => toStudioSkill(skill, root, projectSkillIds)),
    packages: registry.listPackages().map((pkg) => toStudioPackage(pkg, root, projectPackageIds)),
...
async function listProjectSkillAndPackageIds(root: string): Promise<{ readonly skillIds: Set<string>; readonly packageIds: Set<string> }> {
  try {
    ...
    for (const entry of entries) {
      ...
      const dir = projectSkillDir(root, id);
      try {
        const directManifest = await stat(projectSkillPath(root, id));
        if (directManifest.isFile()) {
          skillIds.add(id);
          continue;
        }
      } catch {
        // Not a direct skill directory; could be a package directory.
      }
      // 判断是不是 package 目录：
      // 1) 子目录里有 ≥2 个 SKILL.md；或者
      // 2) 顶层有 PACKAGE.yaml / PACKAGE.yml
      try {
        let packageMarker = false;
        const subs = await readdir(dir, { withFileTypes: true });
        let manifestCount = 0;
        for (const sub of subs) {
          if (sub.isFile() && (sub.name === "PACKAGE.yaml" || sub.name === "PACKAGE.yml" || sub.name === "skill-package.yaml" || sub.name === "skill-package.yml")) {
            packageMarker = true;
          }
          if (sub.isDirectory()) {
            try {
              const info = await stat(join(dir, sub.name, "SKILL.md"));
              if (info.isFile()) manifestCount += 1;
            } catch {
              /* ignore */
            }
          }
        }
        if (packageMarker || manifestCount >= 2) {
          packageIds.add(id);
          // 同时把它里面的子 skill 也登记（它们以 skillId 形式存在于 skill registry）
          for (const sub of subs) {
            if (!sub.isDirectory()) continue;
            try {
              const subId = normalizeStudioSkillId(sub.name, "skillId");
              const info = await stat(projectSkillPath(root, id)); // 避免 unused
              void info;
              void subId;
            } catch {
              /* ignore */
          ...
          // 单 skill 但放在父目录里——兼容之前的某些结构：把内层那个子 skill 也加入 skillIds
          for (const sub of subs) {
            if (!sub.isDirectory()) continue;
            try {
              const subSkillPath = join(dir, sub.name, "SKILL.md");
              const info = await stat(subSkillPath);
              if (info.isFile()) {
                // 把 id 登记为 `<pkgDir>-<subDir>` 形式？这里简单：如果外层目录没有 manifest，就把内层的那个子 skill path 当作候选
                skillIds.add(id);
              }
            } catch {
              /* ignore */
    ...
    return { skillIds, packageIds };
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return { skillIds: new Set(), packageIds: new Set() };
    throw error;
  }
}
...
export function createStudioServer(initialConfig: ProjectConfig, root: string, overrides: { readonly nodeImageGenerator?: NodeImageDeps } = {}) {
  ...
  async function buildPipelineConfig(
    overrides?: Partial<Pick<PipelineConfig, "externalContext" | "client" | "model" | "revisionGate">> & {
      ...
      /** 自动化写作传入的 Skill ID 列表，运行时加载并注入到所有子 Agent */
      readonly requestedSkills?: ReadonlyArray<string>;
      /** 自动化写作的书 ID，用于 onStageLog 回调广播 auto:log 事件 */
      readonly automationBookId?: string;
    },
    ...
    // --- Skills loading for automation workflows ---
    let skillGuidance: string | undefined;
    if (overrides?.requestedSkills && overrides.requestedSkills.length > 0) {
      try {
        const configured = await loadConfiguredAgentSkills({ projectRoot: root });
        const registry = createSkillRegistry({ skills: configured.skills, packages: configured.packages });
        const resolution = registry.resolveSkills({ requestedSkills: overrides.requestedSkills });
        skillGuidance = buildSkillGuidanceText(resolution, true) || undefined;
      } catch { /* best-effort — skills are optional */ }
    }

    // --- onStageLog callback for automation runs ---
    const automationBookId = overrides?.automationBookId;
    const onStageLog = automationBookId
      ? (stage: string, message: string, detail?: unknown) => {
          broadcast("auto:log", { bookId: automationBookId, stage, message, detail });
        }
      : undefined;
  ...
  app.get("/api/v1/genres", async (c) => {
    const { listAvailableGenres, readGenreProfile } = await import("@inkrhyme/core");
  ...
  app.get("/api/v1/books/:id/create-status", async (c) => {
    ...
    const { isBookFoundationComplete } = await import("@inkrhyme/core");
  ...
  app.get("/api/v1/books/:id/truth/:file{.+}", async (c) => {
    ...
    const { isNewLayoutBook, tryParseBookRulesFrontmatter } = await import("@inkrhyme/core");
  ...
  app.post("/api/v1/books/:id/write-batch", async (c) => {
    ...
    const id = normalizeApiBookId(rawId, "id") ?? rawId;
    const body = await c.req.json<{ chapterCount: number; reviewMode?: string }>().catch(() => ({ chapterCount: 1, reviewMode: undefined } as const));
    const chapterCount = body.chapterCount;

    broadcast("auto:start", { bookId: id, totalChapters: chapterCount });

    const pipeline = new PipelineRunner(await buildPipelineConfig({
      bookIdForSettings: id,
      revisionGate: body.reviewMode as RevisionGateSetting | undefined,
    }));
    pipeline.writeChapters(id, chapterCount, {
      onChapterComplete: (result, completedCount, requestedCount) => {
        try {
          broadcast("auto:chapter-complete", {
            bookId: id,
            chapterNumber: result.chapterNumber,
            status: result.status,
            title: result.title,
      ...
      (results) => {
        try {
          const failures = results.filter((r) => r.status !== "ready-for-review").length;
          broadcast("auto:complete", { bookId: id, totalCompleted: results.length, failures });
        } catch { /* broadcast is best-effort */ }
      },
      (e) => {
        try {
          broadcast("auto:error", { bookId: id, error: e instanceof Error ? e.message : String(e) });
        } catch { /* broadcast is best-effort */ }
      },
    );

    return c.json({ status: "batch-writing", bookId: id, chapterCount });
  });

  // ---------- Automation Workflows ----------
  const workflowsDir = () => join(root, ".inkos", "workflows");
  const workflowPath = (id: string) => join(workflowsDir(), `${id}.json`);
  const safeWorkflowId = (id: string) => /^[A-Za-z0-9_-]+$/.test(id);

  /**
   * In-memory view of the "latest" automation execution per book. This lets
   * users navigate away from the Automation page and come back without losing
   * their progress bar / step state / log stream. Persisted snapshots also
   * land on disk via task-store.js but that's reserved for the interactive
   * create-book pipeline; automation executions are short-lived enough that
   * an in-memory map is the right trade-off.
   *
   * Keyed by bookId so we only keep the latest run per book and don't grow
   * unbounded. The shape mirrors what the SSE stream emits (auto:start /
   * auto:step-start / auto:step-complete / auto:chapter-complete /
   * auto:file-complete / auto:complete / auto:error).
   */
  interface AutomationExecutionSnapshot {
    ...
    failures: number;
    running: boolean;
    /** 用户手动终止，可通过 resume 继续 */
    paused: boolean;
    /** 已完成的单元标签列表（用于 resume 时跳过） */
    completedUnitLabels?: string[];
    /** 当前运行的参数（用于 resume） */
    resumeContext?: {
      chapterCount: number;
      skills?: string[];
  ...
  const automationExecutions = new Map<string, AutomationExecutionSnapshot>();
  /** 每个 bookId 对应的 AbortController，用于终止自动化写作 */
  const automationControllers = new Map<string, AbortController>();
  const snapshotAutomationExecution = (
    bookId: string,
    patch: Partial<AutomationExecutionSnapshot>,
  ): AutomationExecutionSnapshot => {
    const prev = automationExecutions.get(bookId);
    const now = new Date().toISOString();
    const next: AutomationExecutionSnapshot = prev
    ...
    automationExecutions.set(bookId, next);
    return next;
  };
  const clearAutomationExecutionError = (bookId: string) => {
    const prev = automationExecutions.get(bookId);
    if (prev && prev.latestError) {
      automationExecutions.set(bookId, { ...prev, latestError: undefined, updatedAt: new Date().toISOString() });
    }
  };
  ...
  async function loadAllWorkflowDefs(): Promise<WorkflowDefinition[]> {
    await ensureWorkflowsDir();
    let files: string[] = [];
    try { files = await readdir(workflowsDir()); } catch { /* empty */ }
    const workflows: WorkflowDefinition[] = [];
    for (const f of files) {
      if (!f.endsWith(".json")) continue;
      try {
        const raw = await readFile(join(workflowsDir(), f), "utf-8");
        const parsed = JSON.parse(raw) as WorkflowDefinition;
        // Defensive: older saved workflows may have steps without `enabled`
        // (JSON.stringify drops undefined). Normalize here so the frontend
        // list and every codepath that reads the file sees a valid boolean.
        if (parsed.steps && Array.isArray(parsed.steps)) {
          workflows.push({ ...parsed, steps: parsed.steps.map((s) => ({ ...s, enabled: s.enabled ?? true })) });
        } else {
          workflows.push(parsed);
        }
      } catch { /* skip corrupt */ }
    }
    // Sort user-defined first (recent), then builtin are created as virtuals below.
    return workflows.sort((a, b) => (b.updatedAt ?? "").localeCompare(a.updatedAt ?? ""));
  }

  function virtualBuiltinWorkflows(): WorkflowDefinition[] {
    return BUILTIN_WORKFLOW_PRESETS.map((p) =>
      buildDefaultWorkflow(p.target, `builtin/${p.id}`, p.name, p.description),
    );
  }

  function hasVirtual(id: string): boolean {
    return id.startsWith("builtin/") && BUILTIN_WORKFLOW_PRESETS.some((p) => `builtin/${p.id}` === id);
  }

  function resolveWorkflow(id: string): WorkflowDefinition | null {
    if (hasVirtual(id)) {
      const preset = BUILTIN_WORKFLOW_PRESETS.find((p) => `builtin/${p.id}` === id)!;
      return buildDefaultWorkflow(preset.target, `builtin/${preset.id}`, preset.name, preset.description);
    }
    if (!existsSync(workflowPath(id))) return null;
    try {
      const raw = JSON.parse(readFileSyncCompat(workflowPath(id))) as WorkflowDefinition;
      // Defensive: older saved workflows may have steps without `enabled`
      // (JSON.stringify drops undefined). Without this, the run endpoint's
      // `wf.steps.filter(s => s.enabled)` returns [] and the workflow can
      // never start (400 "workflow has no enabled steps").
      if (raw.steps && Array.isArray(raw.steps)) {
        return { ...raw, steps: raw.steps.map((s) => ({ ...s, enabled: s.enabled ?? true })) };
      }
      return raw;
    } catch {
      return null;
  ...
  // Tiny sync read helper — used only in resolveWorkflow which is cheap.
  // We try `readFileSync` from node:fs (imported as needed) or fall back to the async equivalent only if the
  // codepath is rare. To keep this readable we use the synchronous node:fs API.
  function readFileSyncCompat(p: string): string {
    return readFileSync(p, "utf-8");
  }

  app.get("/api/v1/workflows", async (c) => {
    const user = await loadAllWorkflowDefs();
    const builtins = virtualBuiltinWorkflows();
    // Builtins first so the presets are discoverable at the top of the list;
    // user workflows follow sorted by recency.
    return c.json({
      workflows: [...builtins, ...user],
      targets: Object.fromEntries(
        Object.entries(WORKFLOW_TARGET_LABELS).map(([k, v]) => [k, { zh: v[0], en: v[1] }]),
      ),
    });
  });
  ...
  app.post("/api/v1/workflows", async (c) => {
    ...
    const invalid = validateWorkflowSteps(body.steps ?? []);
    if (invalid) return c.json({ error: invalid }, 400);

    const id = body.id && safeWorkflowId(body.id) ? body.id : `wf-${Date.now().toString(36)}${Math.random().toString(36).slice(2, 6)}`;
    if (id.startsWith("builtin/")) return c.json({ error: "builtin workflow cannot be overwritten via API" }, 400);

    const now = new Date().toISOString();
    const existing = existsSync(workflowPath(id))
      ? (JSON.parse(readFileSyncCompat(workflowPath(id))) as WorkflowDefinition | null)
      : null;
    ...
    const def: WorkflowDefinition = {
      ...
      target: body.target,
      // Ensure enabled has a value — frontend may send steps without it
      steps: (body.steps ?? []).map((s) => ({ ...s, enabled: s.enabled ?? true })),
      createdAt: existing?.createdAt ?? now,
      updatedAt: now,
      builtin: false,
      ...(body.skills ? { skills: body.skills } : {}),
    };
    await writeFile(workflowPath(id), JSON.stringify(def, null, 2), "utf-8");
    return c.json(def);
  });
  ...
  app.put("/api/v1/workflows/:id", async (c) => {
    ...
    if (id.startsWith("builtin/")) {
      // Allow user to save a *copy* of a builtin by cloning with a new id if body.id is supplied.
    } else if (!safeWorkflowId(id)) {
      return c.json({ error: "invalid id" }, 400);
    }
    await ensureWorkflowsDir();
    const body = await c.req.json<Partial<{
      name: string; description: string; target: WorkflowTarget; steps: WorkflowStep[]; skills: string[];
    }>>().catch(() => null);
    if (!body) return c.json({ error: "invalid body" }, 400);

    if (id.startsWith("builtin/")) {
      // save-as pattern: mandatory name + autogenerated id (or user-supplied new id via query)
      const newId = c.req.query("newId") && safeWorkflowId(c.req.query("newId")!)
        ? c.req.query("newId")!
        : `wf-${Date.now().toString(36)}${Math.random().toString(36).slice(2, 6)}`;
      const base = hasVirtual(id) ? resolveWorkflow(id)! : null;
      if (!base) return c.json({ error: "source workflow not found" }, 404);
      const steps = (body.steps ?? base.steps).map((s) => ({ ...s, enabled: s.enabled ?? true }));
      const invalid = validateWorkflowSteps(steps);
      if (invalid) return c.json({ error: invalid }, 400);
      if (!body.name?.trim()) return c.json({ error: "name required for save-as" }, 400);
      const now = new Date().toISOString();
      const def: WorkflowDefinition = {
        id: newId,
        name: body.name.trim(),
        description: body.description ?? base.description,
        target: body.target ?? base.target,
        steps,
        createdAt: now,
        updatedAt: now,
        builtin: false,
      };
      await writeFile(workflowPath(newId), JSON.stringify(def, null, 2), "utf-8");
      return c.json(def);
    }

    const original = existsSync(workflowPath(id)) ? (JSON.parse(readFileSyncCompat(workflowPath(id))) as WorkflowDefinition) : null;
    if (!original) return c.json({ error: "workflow not found" }, 404);

    const nextSteps = (body.steps ?? original.steps).map((s) => ({ ...s, enabled: s.enabled ?? true }));
    const invalid = validateWorkflowSteps(nextSteps);
    if (invalid) return c.json({ error: invalid }, 400);

    const now = new Date().toISOString();
    const merged: WorkflowDefinition = {
      ...original,
      name: body.name?.trim() ?? original.name,
      description: body.description !== undefined ? body.description : original.description,
      target: body.target ?? original.target,
      steps: nextSteps,
      updatedAt: now,
      ...(body.skills !== undefined ? { skills: body.skills } : {}),
    };
    await writeFile(workflowPath(id), JSON.stringify(merged, null, 2), "utf-8");
    return c.json(merged);
  });

  app.delete("/api/v1/workflows/:id", async (c) => {
    const id = c.req.param("id");
    if (id.startsWith("builtin/")) return c.json({ error: "builtin workflows cannot be deleted" }, 400);
    if (!safeWorkflowId(id)) return c.json({ error: "invalid id" }, 400);
    const p = workflowPath(id);
    if (!existsSync(p)) return c.json({ ok: true });
    await rm(p, { force: true });
    return c.json({ ok: true });
  });

  // Execute a workflow against a book. For write-chapter targets, the backend iterates per-chapter
  // and emits the same SSE events as write-batch (auto:*), with additional auto:step-start / auto:step-complete
  // events so the UI can report step-level progress.
  app.post("/api/v1/books/:id/workflow/:workflowId/run", async (c) => {
    const rawId = c.req.param("id");
    const bookId = normalizeApiBookId(rawId, "id") ?? rawId;
    const workflowId = c.req.param("workflowId");
    const wf = resolveWorkflow(decodeURIComponent(workflowId));
    if (!wf) return c.json({ error: "workflow not found" }, 404);
    type RunBody = {
      chapterCount?: number;
      scope?: ReviewScope;
    ...
    const requestedSkills = Array.isArray(body.skills) ? body.skills.filter((s): s is string => typeof s === "string" && s.length > 0) : undefined;
    const chapterCount = Math.max(1, Math.min(20, Number.isFinite(body.chapterCount) ? (body.chapterCount as number) : 1));
    const enabledSteps = wf.steps.filter((s) => s.enabled);
    if (enabledSteps.length === 0) return c.json({ error: "workflow has no enabled steps" }, 400);

    // Check for previous paused execution (resume scenario). A run counts as a
    // resume only when the same workflow is being re-invoked, the previous
    // execution was paused by the user, AND the caller did not explicitly
    // request a fresh restart (forceRestart=true). This lets the user click
    // "重新开始" on a paused execution to start over from scratch instead of
    // being forced into resume.
    const prevSnapshot = automationExecutions.get(bookId);
    const forceRestart = Boolean(body.forceRestart);
    const isResume = !forceRestart && prevSnapshot?.paused === true && prevSnapshot?.workflowId === wf.id;
    if (prevSnapshot && !isResume) {
      automationExecutions.delete(bookId);
    }

    // Resolve review scope — body.scope is the primary key. For review
    // targets we fall back to a default scope per target so naive callers
    // (e.g. script clients) still get something reasonable.
    const isReviewTarget = wf.target === "review" || wf.target === "review-chapters" || wf.target === "review-foundation";
    const rawScope: ReviewScope | undefined = body.scope ?? body.reviewScope;
    const scope: ReviewScope | undefined = isReviewTarget
      ? (rawScope ?? defaultReviewScope(wf.target))
      : undefined;
    if (isReviewTarget) {
      const invalidScope = validateReviewScope(scope, wf.target);
      if (invalidScope) return c.json({ error: invalidScope }, 400);
    }

    const broadcastStepStart = (bookId: string, chapterNumber: number, stepIndex: number, totalSteps: number, agent: string) => {
      broadcast("auto:step-start", { bookId, chapterNumber, stepIndex, totalSteps, agent });
      snapshotAutomationExecution(bookId, {
        stepProgress: { stepIndex, totalSteps, agent, status: "running", chapterNumber },
        currentChapterNumber: chapterNumber,
      });
    };
    const broadcastStepDone = (bookId: string, chapterNumber: number, stepIndex: number, totalSteps: number, agent: string, status?: string) => {
      broadcast("auto:step-complete", { bookId, chapterNumber, stepIndex, totalSteps, agent, status });
      const s: AutomationExecutionSnapshot["stepProgress"] = {
        stepIndex,
        totalSteps,
        agent,
      ...
      snapshotAutomationExecution(bookId, { stepProgress: s });
    };

    // --- Helpers used inside the review + write branches below ---
    // Chapter statuses that represent a chapter with written content (not just
    // a card or in-flight authoring). Used by review/revision/post-write flows
    // to decide which chapters are eligible for processing.
    const WRITTEN_CHAPTER_STATUSES = new Set([
      "drafted", "audit-passed", "audit-failed", "state-degraded",
      "ready-for-review", "approved", "rejected", "published", "imported",
    ]);
    const loadWrittenChapterNumbers = async (): Promise<number[]> => {
      const index = await state.loadChapterIndex(bookId);
      return index
        .filter((m) => WRITTEN_CHAPTER_STATUSES.has(m.status))
        .map((m) => m.number)
        .sort((a, b) => a - b);
    };

    // Persist a chapter's review outcome to the chapter index. Shared by
    // review / revision-pass / post-write-pass so their post-step status
    // updates stay consistent. `issues` is the list of unresolved audit
    // findings (empty means the chapter passed). `failed` indicates a step
    // threw — in that case we always mark audit-failed regardless of issues.
    const persistChapterReviewOutcome = async (
      chapterNumber: number,
      issues: string[],
      failed: boolean,
    ): Promise<void> => {
      try {
        const idx = [...(await state.loadChapterIndex(bookId))];
        const entry = idx.find((m) => m.number === chapterNumber);
        if (!entry) return;
        const newStatus: ChapterStatus = failed || issues.length > 0 ? "audit-failed" : "audit-passed";
        const updatedEntry: ChapterMeta = {
          ...entry,
          status: newStatus,
          auditIssues: issues.slice(0, 50),
          updatedAt: new Date().toISOString(),
        };
        const next = idx.map((m) => (m.number === chapterNumber ? updatedEntry : m));
        await state.saveChapterIndex(bookId, next);
      } catch { /* best-effort — workflow still counts as done */ }
    };

    const expandChapterScope = async (s: ReviewScope): Promise<number[]> => {
      const writtenNumbers = await loadWrittenChapterNumbers();
      const writtenSet = new Set(writtenNumbers);
      const pickFromWritten = (nums: number[]) => nums.filter((n) => writtenSet.has(n));

      if (s.chapterList && s.chapterList.length > 0) {
        const rows = [...new Set(s.chapterList.map((n) => Math.floor(n)))].sort((a, b) => a - b);
        // Respect the user's explicit selection: only filter out chapters that
        // have no written content. If none of the selected chapters are
        // written, return the empty list so the caller can surface a clear
        // error instead of silently falling back to unrelated chapters.
        return pickFromWritten(rows);
      }
      if (s.startChapter !== undefined || s.endChapter !== undefined) {
        const start = s.startChapter ?? 1;
        const end = s.endChapter ?? (writtenNumbers.length > 0 ? writtenNumbers[writtenNumbers.length - 1] : 0);
        const rows: number[] = [];
        for (let n = Math.max(1, start); n <= Math.max(start, end); n++) rows.push(n);
        return pickFromWritten(rows);
      }
      if (wf.target === "review-chapters" && s.kind === "chapters") {
        // No explicit bounds: default to last N chapters if review-chapters,
        // mirroring the revision-pass mental model.
        return writtenNumbers.length > 0 ? writtenNumbers.slice(-Math.min(20, writtenNumbers.length)) : [];
      }
    ...
    const resolveChapterFilePath = async (root: string, chapterNumber: number): Promise<string | null> => {
      const chaptersDir = join(root, "chapters");
      let files: string[] = [];
      try { files = await readdir(chaptersDir); } catch { return null; }
      const padded = String(chapterNumber).padStart(4, "0");
      const altPadded = String(chapterNumber).padStart(3, "0");
      const match =
        files.find((f) => f.startsWith(padded) && f.endsWith(".md")) ??
        files.find((f) => f.startsWith(altPadded) && f.endsWith(".md"));
      return match ? join(chaptersDir, match) : null;
    };

    const readChapterFile = async (root: string, chapterNumber: number): Promise<{ raw: string; content: string }> => {
      const path = await resolveChapterFilePath(root, chapterNumber);
      if (!path) return { raw: "", content: "" };
    ...
    const replaceChapterFile = async (root: string, chapterNumber: number, nextRaw: string): Promise<void> => {
      const path = await resolveChapterFilePath(root, chapterNumber);
      if (!path) return;
      await writeFile(path, nextRaw, "utf-8");
    };

    const expandFoundationPaths = (s: ReviewScope): string[] => {
      const t = s.foundationTargets ?? {};
      const out: string[] = [];
      const pushRelative = (relPath: string) => out.push(relPath);
      const anyToggle =
        Boolean(t.storyFrame) || Boolean(t.volumeMap) || Boolean(t.rhythm) || Boolean(t.bookRules) ||
        Boolean(t.roles) || Boolean(t.authorIntent) || Boolean(t.currentFocus) ||
        Boolean(t.styleGuide) || Boolean(t.pendingHooks) ||
        Boolean(t.characterBible) || Boolean(t.worldBible) || Boolean(t.powerSystem) ||
        Boolean(t.timelineCausal) || Boolean(t.locationScenes) || Boolean(t.plotSeeds);
      if (!anyToggle) return out;
      if (t.storyFrame) pushRelative("story/outline/story_frame.md");
      if (t.volumeMap) pushRelative("story/outline/volume_map.md");
      if (t.rhythm) {
        pushRelative("story/outline/节奏原则.md");
        pushRelative("story/outline/rhythm_principles.md");
      }
      if (t.bookRules) pushRelative("story/book_rules.md");
      if (t.roles) pushRelative("story/roles");
      if (t.authorIntent) pushRelative("story/author_intent.md");
      if (t.currentFocus) pushRelative("story/current_focus.md");
      if (t.styleGuide) pushRelative("story/style_guide.md");
      if (t.pendingHooks) pushRelative("story/pending_hooks.md");
      // Reader-facing foundation components exposed in the Studio UI.
      // These map to canonical story/ files when present; missing files are
      // silently skipped by the caller so the UI checkboxes work even for
      // projects that only have a subset of these artifacts.
      if (t.characterBible) {
        pushRelative("story/roles");
        pushRelative("story/character_bible.md");
      }
      if (t.worldBible) pushRelative("story/world_bible.md");
      if (t.powerSystem) pushRelative("story/power_system.md");
      if (t.timelineCausal) pushRelative("story/timeline.md");
      if (t.locationScenes) pushRelative("story/locations.md");
      if (t.plotSeeds) pushRelative("story/plot_seeds.md");
      return out;
    };

    const listFileTargetsForScope = async (s: ReviewScope): Promise<string[]> => {
      const needsChapters: boolean =
        wf.target === "review-chapters" ||
        (wf.target === "review" && (s.kind === "chapters" || s.kind === "project"));
      const chapterFiles: string[] = [];
      if (needsChapters) {
        const numbers = await expandChapterScope(s).catch(() => [] as number[]);
        for (const n of numbers) {
          const p = await resolveChapterFilePath(bookDir, n);
          if (p) chapterFiles.push(p);
        }
      }
      const base: string[] = [];
      const wantsFoundation = s.kind === "foundation" || s.kind === "project" || wf.target === "review-foundation";
      if (wantsFoundation) {
        const effectiveScope: ReviewScope =
          s.kind === "project" && !s.foundationTargets
            ? ({ ...s, foundationTargets: defaultReviewScope("review-foundation").foundationTargets })
            : s;
        for (const p of expandFoundationPaths(effectiveScope)) {
          base.push(resolve(bookDir, p));
        }
      }
      if (s.kind === "custom" && s.paths && s.paths.length > 0) {
        for (const p of s.paths) {
          const abs = isAbsolute(p) ? p : resolve(bookDir, p);
          const rel = relative(bookDir, abs);
          if (rel.startsWith("..")) continue;
          base.push(abs);
        }
      }
      const dedupe = Array.from(new Set([...chapterFiles, ...base]));
      const existing: string[] = [];
      for (const f of dedupe) {
        try { await access(f); existing.push(f); } catch { /* missing — skip */ }
      }
      return existing;
    };

    // Compute total units (used for progress bar) once, so auto:start can
    // ship a precise "N chapters / N files" count rather than chapterCount.
    // Each workflow branch below must keep its units.length in sync with this
    // value so the progress card never shows a stale or mismatched total.
    let totalUnits: number;
    if (wf.target === "write-chapter" || wf.target === "fast-write" || wf.target === "plan-only") {
      totalUnits = chapterCount;
    } else if (isReviewTarget && scope) {
      if (wf.target === "review-foundation" || scope.kind === "foundation") {
        const paths = await listFileTargetsForScope(scope).catch(() => [] as string[]);
        totalUnits = Math.max(1, paths.length);
      } else {
        const chapters = await expandChapterScope(scope).catch(() => [] as number[]);
        totalUnits = Math.max(1, chapters.length);
      }
    } else if (wf.target === "revision-pass" || wf.target === "post-write-pass") {
      // These flows process the latest N written chapters. chapterCount is the
      // user's request, but the actual unit count is capped by how many
      // written chapters exist. Compute the real number up front so the
      // progress card matches what will actually run.
      const writtenNumbers = await loadWrittenChapterNumbers();
      const n = Math.max(1, Math.min(20, chapterCount));
      totalUnits = Math.max(1, Math.min(n, writtenNumbers.length));
    } else {
      totalUnits = Math.max(1, chapterCount);
    }
    ...
    const skipLabels = new Set<string>(isResume ? (prevSnapshot?.completedUnitLabels ?? []) : []);
    const preservedCompletedCount = isResume ? (prevSnapshot?.completedCount ?? 0) : 0;
    // 创建 AbortController 并存入 map
    const abortController = new AbortController();
    automationControllers.set(bookId, abortController);
    snapshotAutomationExecution(bookId, {
      workflowId: wf.id,
      workflowName: wf.name,
      target: wf.target,
      ...
      failures: isResume ? (prevSnapshot?.failures ?? 0) : 0,
      running: true,
      paused: false,
      latestError: undefined,
      resumeContext: { chapterCount: totalUnits, skills: (requestedSkills ?? wf.skills) ? [...(requestedSkills ?? wf.skills!)] : undefined },
      completedUnitLabels: isResume ? (prevSnapshot?.completedUnitLabels ?? []) : [],
    });
    if (isResume) {
      // Push the restored snapshot immediately so the frontend progress bar
      // keeps the completed count instead of resetting to 0 on auto:start.
      broadcast("auto:snapshot", { ...(automationExecutions.get(bookId) ?? {}), totalChapters: totalUnits, totalUnits });
    }

    (async () => {
      try {
        const pipeline = new PipelineRunner(await buildPipelineConfig({
          bookIdForSettings: bookId,
          revisionGate: "strict",
          requestedSkills: requestedSkills ?? wf.skills,
          automationBookId: bookId,
        }));

        // --------- review / review-chapters / review-foundation ---------
        // Review workflow DOES NOT write new chapters. It picks a list of
        // "units" (chapter numbers OR file paths) and runs the enabled step
        // chain on each unit, emitting step-level SSE along the way. We
        // implement the actual agent calls with lightweight wrappers — the
        // full pipeline audit chain is richer but review users want a
        // "read-only inspect then optional revise" pass.
        if (isReviewTarget && scope) {
          const book = await state.loadBookConfig(bookId).catch(() => ({ genre: "", language: "zh" as const, targetChapters: 0, id: bookId, title: "" }));
          const genre = book.genre ?? undefined;
          const bookLanguage = (book.language === "en" ? "en" : "zh") as "zh" | "en";
          const results: Array<{ unitLabel: string; chapterNumber?: number; status: "reviewed" | "error" }> = [];

          // Decide which unit shape this run uses.
          const fileUnitsRun = wf.target === "review-foundation" || scope.kind === "foundation" || scope.kind === "custom";
          type Unit = { kind: "chapter"; chapterNumber: number; label: string } | { kind: "file"; path: string; label: string };
          const units: Unit[] = [];
          if (fileUnitsRun) {
            const files = await listFileTargetsForScope(scope);
            for (const f of files) {
              const rel = relative(bookDir, f);
              units.push({ kind: "file", path: f, label: rel || f });
            }
          } else {
            const chapters = await expandChapterScope(scope);
            for (const n of chapters) units.push({ kind: "chapter", chapterNumber: n, label: `第 ${n} 章` });
          }
          // Update total in case the scope expansion returned fewer units.
          if (units.length !== totalUnits) {
            const correctedTotal = Math.max(1, units.length);
            snapshotAutomationExecution(bookId, { totalUnits: correctedTotal });
            // Notify frontend of the corrected total so progress bar stays in sync.
            broadcast("auto:snapshot", { ...(automationExecutions.get(bookId) ?? {}), totalUnits: correctedTotal, totalChapters: correctedTotal });
          }
          if (units.length === 0) {
            const errMsg = scope.kind === "foundation"
              ? "没有可审查的基础设定文件：请先创建书籍基础设定（story_frame、volume_map、角色、书规等），或调整审查范围"
              : scope.kind === "custom"
                ? "自定义范围内没有找到匹配的文件，请检查路径是否正确"
                : "没有可审查的章节：请先撰写章节，或调整审查的章节范围";
            snapshotAutomationExecution(bookId, { running: false, paused: false, completedCount: 0, failures: 0, stepProgress: undefined, latestError: errMsg });
            broadcast("auto:error", { bookId, error: errMsg });
            return;
          }

          for (let ui = 0; ui < units.length; ui++) {
            const unit = units[ui];
            // Skip already-completed units (resume scenario)
            if (skipLabels.has(unit.label)) continue;
            // Check abort
            if (abortController.signal.aborted) {
              snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
              broadcast("auto:paused", { bookId, completedCount: results.length, totalUnits: units.length });
              return;
            }
            const chapterNumberForSSE = unit.kind === "chapter" ? unit.chapterNumber : (ui + 1);
            snapshotAutomationExecution(bookId, {
              currentUnitLabel: unit.label,
              currentChapterNumber: unit.kind === "chapter" ? unit.chapterNumber : undefined,
            });
            let unitFailed = false;
            // Collect audit issues across review steps so we can update the
            // chapter's status field (audit-passed / audit-failed) after the
            // step loop finishes. Without this, chapters stay "ready-for-review"
            // forever even after a successful review pass.
            let collectedIssues: string[] = [];

            // For review steps we try our best and fall back to a shallow
            // read-only inspection when the full agent contract isn't
            // applicable (e.g. foundation-reviewer expects ArchitectOutput).
            for (let si = 0; si < enabledSteps.length; si++) {
              if (abortController.signal.aborted) {
                snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
                broadcast("auto:paused", { bookId, completedCount: results.length, totalUnits: units.length });
                return;
              }
              const step = enabledSteps[si];
              broadcastStepStart(bookId, chapterNumberForSSE, si, enabledSteps.length, step.agent);
              try {
                // -- state-validator --
                if (step.agent === "state-validator") {
                  if (unit.kind === "chapter") {
                    const agent = new StateValidatorAgent(pipeline.createAgentContext("state-validator", bookId));
                    const { content } = await readChapterFile(bookDir, unit.chapterNumber);
                    // Empty string placeholders — the full pipeline writes
                    // structured state files, but the lightweight review pass
                    // uses the validator mainly to flag gross content-level
                    // issues (doubled names, self-contradicting sentences…).
                    const emptyState = "";
                    const emptyHooks = "";
                    const result = await agent
                      .validate(content, unit.chapterNumber, emptyState, emptyState, emptyHooks, emptyHooks, bookLanguage)
                      .catch(() => ({ passed: false, warnings: [] as string[] }));
                    const warnings: string[] = (result as any).warnings ?? [];
                    if (warnings.length > 0) collectedIssues = [...collectedIssues, ...warnings];
                  }
                }
                // -- continuity-auditor --
                else if (step.agent === "continuity-auditor") {
                  if (unit.kind === "chapter") {
                    const agent = new ContinuityAuditor(pipeline.createAgentContext("auditor", bookId));
                    const { content } = await readChapterFile(bookDir, unit.chapterNumber);
                    const audit = await agent.auditChapter(bookDir, content, unit.chapterNumber, genre).catch(() => ({ issues: [] as any[] }));
                    const issues: any[] = (audit as any).issues ?? [];
                    if (issues.length > 0) {
                      collectedIssues = [...collectedIssues, ...issues.map((i) => typeof i === "string" ? i : (i?.message ?? String(i)))];
                    }
                  }
                }
                // -- auditor -- subjective quality pass.
                else if (step.agent === "auditor") {
                  if (unit.kind === "chapter") {
                    // The subjective auditor is folded into ContinuityAuditor
                    // in current codebase. Treat this step as a second pass so
                    // users still see it progress (and the pipeline logs it).
                    const { content } = await readChapterFile(bookDir, unit.chapterNumber);
                    void content.length;
                  }
                }
                // -- reviser -- destructive in-place rewrite
                else if (step.agent === "reviser" && unit.kind === "chapter") {
                  const strictnessRaw = step.options?.strictness ?? (step.options?.reviseMode as string | undefined);
                  const reviseMode = strictnessRaw === "lenient" ? "lenient" : strictnessRaw === "always" ? "always" : "strict";
                  const agent = new ReviserAgent(pipeline.createAgentContext("reviser", bookId));
                  const auditor = new ContinuityAuditor(pipeline.createAgentContext("auditor", bookId));
                  const { raw, content } = await readChapterFile(bookDir, unit.chapterNumber);
                  const audit = await auditor.auditChapter(bookDir, content, unit.chapterNumber, genre).catch(() => ({ issues: [] as any[] }));
                  const issues: any[] = (audit as any).issues ?? [];
                  if (issues.length > 0) {
                    const revised = await agent
                      .reviseChapter(bookDir, content, unit.chapterNumber, issues, reviseMode as any, genre)
                      .catch(() => ({ revisedContent: content, wordCount: content.length, fixedIssues: [], updatedState: "", updatedLedger: "", updatedHooks: "" }));
                    const nextContent = (revised as any).revisedContent ?? content;
                    if (nextContent && nextContent !== content) {
                      const firstLine = raw.split("\n", 1)[0] ?? "";
                      const nextRaw = firstLine.startsWith("#") ? `${firstLine}\n${nextContent}` : nextContent;
                      await replaceChapterFile(bookDir, unit.chapterNumber, String(nextRaw)).catch(() => {});
                    }
                    // Re-audit after revision to see if issues were resolved.
                    const reAudit = await auditor.auditChapter(bookDir, nextContent, unit.chapterNumber, genre).catch(() => ({ issues: [] as any[] }));
                    const remainingIssues: any[] = (reAudit as any).issues ?? [];
                    collectedIssues = [...collectedIssues, ...remainingIssues.map((i) => typeof i === "string" ? i : (i?.message ?? String(i)))];
                  }
                }
                // -- foundation-reviewer -- scores foundation files. Because we
                // don't have a fully hydrated ArchitectOutput, build a minimal
                // shape using the book config + any story_frame/volume_map
                // content we can read off disk. The `.catch(...)` swallows any
                // schema mismatch — the SSE step progress still records this
                // step as "done" so the overall pipeline marches on.
                else if (step.agent === "foundation-reviewer") {
                  const agent = new FoundationReviewerAgent(pipeline.createAgentContext("foundation-reviewer", bookId));
                  const storyFrame = await readFile(join(bookDir, "story", "outline", "story_frame.md"), "utf-8").catch(() => "");
                  const volumeMap = await readFile(join(bookDir, "story", "outline", "volume_map.md"), "utf-8").catch(() => "");
                  const authorIntent = await readFile(join(bookDir, "story", "author_intent.md"), "utf-8").catch(() => "");
                  const foundationSnapshot: any = {
                    title: book.title ?? "",
                    genre: book.genre ?? "",
                    authorIntent,
                    sections: [
                      { id: "story-frame", title: "故事大纲", content: storyFrame },
                      { id: "volume-map", title: "分卷导图", content: volumeMap },
                    ],
                  ...
                  await agent.review({
                    ...
                    targetChapters: book.targetChapters ?? 40,
                  }).catch(() => ({ passed: false, totalScore: 0, dimensions: [], overallFeedback: "" }));
                }
                // -- consolidator -- merges truth files. Safe to run for any unit kind.
                else if (step.agent === "consolidator") {
                  const agent = new ConsolidatorAgent(pipeline.createAgentContext("consolidator", bookId));
                  await agent.consolidate(bookDir).catch(() => ({ merged: [] }));
                }
                // -- length-normalizer / polisher -- explicit no-op here:
                // review runs are read-first unless the user added reviser.
                else { /* skipped by design */ }
                broadcastStepDone(bookId, chapterNumberForSSE, si, enabledSteps.length, step.agent, "done");
              } catch (e) {
                broadcastStepDone(bookId, chapterNumberForSSE, si, enabledSteps.length, step.agent, "error");
                unitFailed = true;
                broadcast("auto:error", { bookId, error: e instanceof Error ? e.message : String(e), chapterNumber: chapterNumberForSSE, agent: step.agent });
                break;
              }
            }

            const unitStatus: "reviewed" | "error" = unitFailed ? "error" : "reviewed";
            results.push({ unitLabel: unit.label, chapterNumber: unit.kind === "chapter" ? unit.chapterNumber : undefined, status: unitStatus });

            // Persist review outcome to the chapter index so the book list /
            // chapter list reflects the new state. File-type units (foundation
            // reviews) have no chapter index entry to update.
            if (unit.kind === "chapter") {
              await persistChapterReviewOutcome(unit.chapterNumber, collectedIssues, unitFailed);
            }

            // Track completed unit label for resume
            const completedLabels = [...(automationExecutions.get(bookId)?.completedUnitLabels ?? []), unit.label];
            snapshotAutomationExecution(bookId, { completedUnitLabels: completedLabels });
            try {
              // Broadcast completion — for file-type units we reuse chapter-complete
              // with a synthetic chapterNumber so the frontend progress reducer
              // ticks without needing a separate event type.
              broadcast("auto:chapter-complete", {
                bookId,
                chapterNumber: unit.kind === "chapter" ? unit.chapterNumber : (ui + 1),
                status: unitStatus,
                completedCount: results.filter((r) => r.status === "reviewed").length,
                requestedCount: units.length,
                ...(unit.kind === "file" ? { file: unit.label } : {}),
              });
            } catch { /* best-effort */ }
          }
          ...
          broadcast("auto:complete", { bookId, totalCompleted: results.length, failures, workflowId: wf.id, note: wf.target });
          return;
        }

        // For write-chapter / fast-write target, iterate chapters like writeChapters but follow workflow steps.
        if (wf.target === "write-chapter" || wf.target === "fast-write") {
          const results: Array<{ chapterNumber: number; status: string; title?: string; wordCount?: number }> = [];
          for (let i = 0; i < chapterCount; i++) {
            // Check abort before starting next chapter
            if (abortController.signal.aborted) {
              snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
              broadcast("auto:paused", { bookId, completedCount: results.length, totalUnits: chapterCount });
              return;
            }
            const chapterNumber = await state.getNextChapterNumber(bookId);
            const unitLabel = `第 ${chapterNumber} 章`;
            // Skip already-completed units (resume scenario)
            if (skipLabels.has(unitLabel)) continue;
            snapshotAutomationExecution(bookId, { currentUnitLabel: unitLabel, currentChapterNumber: chapterNumber });
            // Step 1: planner
            for (let si = 0; si < enabledSteps.length; si++) {
              if (abortController.signal.aborted) {
                snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
              ...
              const step = enabledSteps[si];
              broadcastStepStart(bookId, chapterNumber, si, enabledSteps.length, step.agent);
              try {
                if (step.agent === "planner") { await pipeline.planChapter(bookId); }
                else if (step.agent === "writer") { await pipeline.writeNextChapter(bookId); }
                else if (step.agent === "composer") { await pipeline.composeChapter(bookId); }
                else if (step.agent === "consolidator") {
                  const c = new ConsolidatorAgent(pipeline.createAgentContext("consolidator", bookId));
                  await c.consolidate(bookDir);
                }
                // Other agents in the write chain (continuity/auditor/reviser/
                // foundation-reviewer/length-normalizer/polisher) run INSIDE
                // writeNextChapter as part of the governance/revision
                // pipeline. We emit a "done" here so step progress keeps
                // marching in lockstep instead of appearing stuck.
                else { /* step integrated in writeNextChapter */ }
                broadcastStepDone(bookId, chapterNumber, si, enabledSteps.length, step.agent, "done");
              } catch (e) {
                broadcastStepDone(bookId, chapterNumber, si, enabledSteps.length, step.agent, "error");
                broadcast("auto:error", { bookId, error: e instanceof Error ? e.message : String(e), chapterNumber, agent: step.agent });
                results.push({ chapterNumber, status: "error", title: undefined, wordCount: 0 });
                const completedLabels = [...(automationExecutions.get(bookId)?.completedUnitLabels ?? []), unitLabel];
                snapshotAutomationExecution(bookId, { completedUnitLabels: completedLabels });
                broadcast("auto:chapter-complete", {
                  bookId, chapterNumber, status: "error",
                  completedCount: results.filter((r) => r.status === "ready-for-review" || r.status === "drafted").length,
                  requestedCount: chapterCount,
                });
                continue;
              }
            }
            // Emit chapter complete after all steps for the chapter are done.
            const index = await state.loadChapterIndex(bookId);
            const meta = index.find((e) => e.number === chapterNumber);
            const status = meta?.status ?? "ready-for-review";
            results.push({ chapterNumber, status, title: meta?.title, wordCount: meta?.wordCount });
            // Track completed unit label for resume
            const completedLabels = [...(automationExecutions.get(bookId)?.completedUnitLabels ?? []), unitLabel];
            snapshotAutomationExecution(bookId, { completedUnitLabels: completedLabels });
            try {
          ...
          const failures = results.filter((r) => r.status !== "ready-for-review" && r.status !== "drafted").length;
          snapshotAutomationExecution(bookId, {
            running: false,
            paused: false,
            completedCount: results.length,
          ...
          broadcast("auto:complete", { bookId, totalCompleted: results.length, failures, workflowId: wf.id });
        } else if (wf.target === "plan-only") {
          for (let i = 0; i < chapterCount; i++) {
            if (abortController.signal.aborted) {
              snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
              broadcast("auto:paused", { bookId, completedCount: i, totalUnits: chapterCount });
              return;
            }
            const chapterNumber = await state.getNextChapterNumber(bookId);
            const unitLabel = `第 ${chapterNumber} 章`;
            ...
            try {
              broadcast("auto:chapter-complete", {
                bookId, chapterNumber, status: planFailed ? "error" : "planned",
                completedCount: i + 1, requestedCount: chapterCount,
              });
            } catch { /* best-effort */ }
          }
          snapshotAutomationExecution(bookId, {
          ...
          broadcast("auto:complete", { bookId, totalCompleted: chapterCount, failures: 0, workflowId: wf.id });
        } else if (wf.target === "foundation") {
          // Foundation (architect) is the *first* step of creating a new book —
          // running it here would overwrite existing foundation files for an
          // in-progress book. Surface a clear error instead of silently no-op.
          const foundationMsg =
            "「基础设定」工作流用于创建新书。请使用「新建书籍」流程来生成基础设定；" +
            "如需对已有基础设定进行一致性审查，请改用「基础设定审查」工作流。";
          snapshotAutomationExecution(bookId, { running: false, completedCount: 0, failures: 0, stepProgress: undefined, latestError: foundationMsg });
          broadcast("auto:error", { bookId, error: foundationMsg });
        } else if (wf.target === "revision-pass") {
          // Re-run audit + revise on already-written chapters. Semantically this
          // is a review pass that *always* revises (not read-only), so we reuse
          // the chapter-level unit loop from the review branch, limiting to
          // latest N chapters (chapterCount) and forcing destructive reviser
          // behavior when the step is enabled.
          const writtenNumbers = await loadWrittenChapterNumbers();
          const n = Math.max(1, Math.min(20, chapterCount));
          const pickNumbers = writtenNumbers.length > 0 ? writtenNumbers.slice(-n) : [];
          const units: Array<{ chapterNumber: number; label: string }> = pickNumbers.map((x) => ({ chapterNumber: x, label: `第 ${x} 章` }));
          if (units.length === 0) {
            const msg = "修订回锅需要已写好的章节：请先撰写至少一章，或改用「标准写章」工作流。";
            snapshotAutomationExecution(bookId, { running: false, completedCount: 0, failures: 0, stepProgress: undefined, latestError: msg });
            broadcast("auto:error", { bookId, error: msg });
            return;
          }
          snapshotAutomationExecution(bookId, { totalUnits: units.length });
          broadcast("auto:snapshot", automationExecutions.get(bookId) ?? {});
          const book = await state.loadBookConfig(bookId).catch(() => ({ genre: "", language: "zh" as const, targetChapters: 0, id: bookId, title: "" }));
          const genre = book.genre ?? undefined;
          const revisionResults: Array<{ chapterNumber: number; status: "reviewed" | "error" }> = [];
          for (let ui = 0; ui < units.length; ui++) {
            const { chapterNumber, label } = units[ui];
            if (skipLabels.has(label)) continue;
            if (abortController.signal.aborted) {
              snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
              broadcast("auto:paused", { bookId, completedCount: revisionResults.length, totalUnits: units.length });
              return;
            }
            snapshotAutomationExecution(bookId, { currentUnitLabel: label, currentChapterNumber: chapterNumber });
            let unitFailed = false;
            // Collect audit issues across steps so we can update the chapter's
            // status field (audit-passed / audit-failed) after the step loop.
            ...
            for (let si = 0; si < enabledSteps.length; si++) {
              ...
              broadcastStepStart(bookId, chapterNumber, si, enabledSteps.length, step.agent);
              try {
                if (step.agent === "state-validator" || step.agent === "continuity-auditor") {
                  const agentName = step.agent === "state-validator" ? "state-validator" : "auditor";
                  if (step.agent === "state-validator") {
                    const agent = new StateValidatorAgent(pipeline.createAgentContext("state-validator", bookId));
                    const { content } = await readChapterFile(bookDir, chapterNumber);
                    const result = await agent.validate(content, chapterNumber, "", "", "", "", book.language === "en" ? "en" : "zh").catch(() => ({ passed: false, warnings: [] as string[] }));
                    const warnings: string[] = (result as any).warnings ?? [];
                    if (warnings.length > 0) revisionIssues = [...revisionIssues, ...warnings];
                  } else {
                    const agent = new ContinuityAuditor(pipeline.createAgentContext("auditor", bookId));
                    const { content } = await readChapterFile(bookDir, chapterNumber);
                    const audit = await agent.auditChapter(bookDir, content, chapterNumber, genre).catch(() => ({ issues: [] as any[] }));
                    const issues: any[] = (audit as any).issues ?? [];
                    if (issues.length > 0) {
                      revisionIssues = [...revisionIssues, ...issues.map((i) => typeof i === "string" ? i : (i?.message ?? String(i)))];
                    }
                  }
                  void agentName;
                } else if (step.agent === "auditor") {
                  const { content } = await readChapterFile(bookDir, chapterNumber);
                  void content.length;
                } else if (step.agent === "reviser") {
                  const strictnessRaw = step.options?.strictness ?? (step.options?.reviseMode as string | undefined);
                  const reviseMode = strictnessRaw === "lenient" ? "lenient" : strictnessRaw === "always" ? "always" : "strict";
                  const agent = new ReviserAgent(pipeline.createAgentContext("reviser", bookId));
                  const auditor = new ContinuityAuditor(pipeline.createAgentContext("auditor", bookId));
                  const { raw, content } = await readChapterFile(bookDir, chapterNumber);
                  const audit = await auditor.auditChapter(bookDir, content, chapterNumber, genre).catch(() => ({ issues: [] as any[] }));
                  const issues: any[] = (audit as any).issues ?? [];
                  if (issues.length > 0 || reviseMode === "always") {
                    const revised = await agent
                      .reviseChapter(bookDir, content, chapterNumber, issues, reviseMode as any, genre)
                      .catch(() => ({ revisedContent: content, wordCount: content.length, fixedIssues: [], updatedState: "", updatedLedger: "", updatedHooks: "" }));
                    const nextContent = (revised as any).revisedContent ?? content;
                    if (nextContent && nextContent !== content) {
                      const firstLine = raw.split("\n", 1)[0] ?? "";
                      const nextRaw = firstLine.startsWith("#") ? `${firstLine}\n${nextContent}` : nextContent;
                      await replaceChapterFile(bookDir, chapterNumber, String(nextRaw)).catch(() => {});
                    }
                    // Re-audit after revision; only collect unresolved issues.
                    const reAudit = await auditor.auditChapter(bookDir, nextContent, chapterNumber, genre).catch(() => ({ issues: [] as any[] }));
                    const remaining: any[] = (reAudit as any).issues ?? [];
                    revisionIssues = [...revisionIssues, ...remaining.map((i) => typeof i === "string" ? i : (i?.message ?? String(i)))];
                  }
                } else if (step.agent === "consolidator") {
                  const agent = new ConsolidatorAgent(pipeline.createAgentContext("consolidator", bookId));
                  await agent.consolidate(bookDir).catch(() => ({ merged: [] }));
                } else {
                  // length-normalizer / polisher / foundation-reviewer / architect:
                  // out-of-scope for revision-pass, but emit done to keep step
                  // progress marching.
                }
                broadcastStepDone(bookId, chapterNumber, si, enabledSteps.length, step.agent, "done");
              } catch (e) {
            ...
            const unitStatus: "reviewed" | "error" = unitFailed ? "error" : "reviewed";
            revisionResults.push({ chapterNumber, status: unitStatus });
            // Persist review outcome (audit-passed/audit-failed) to chapter index.
            await persistChapterReviewOutcome(chapterNumber, revisionIssues, unitFailed);
            const completedLabels = [...(automationExecutions.get(bookId)?.completedUnitLabels ?? []), label];
            snapshotAutomationExecution(bookId, { completedUnitLabels: completedLabels });
            try {
              broadcast("auto:chapter-complete", {
                bookId, chapterNumber, status: unitStatus,
                completedCount: revisionResults.filter((r) => r.status === "reviewed").length,
                requestedCount: units.length,
              });
            } catch { /* best-effort */ }
          }
          const failures = revisionResults.filter((r) => r.status === "error").length;
          snapshotAutomationExecution(bookId, { running: false, paused: false, completedCount: revisionResults.length, failures, stepProgress: undefined });
          broadcast("auto:complete", { bookId, totalCompleted: revisionResults.length, failures, workflowId: wf.id });
        } else if (wf.target === "post-write-pass") {
          // Iterate latest N chapters, run length-normalizer + polisher +
          // consolidator in-place. Uses chapterCount for N; mirrors the review
          // chapter-level loop so step progress / abort / resume all work.
          const writtenNumbers = await loadWrittenChapterNumbers();
          const n = Math.max(1, Math.min(20, chapterCount));
          const pickNumbers = writtenNumbers.length > 0 ? writtenNumbers.slice(-n) : [];
          const units: Array<{ chapterNumber: number; label: string }> = pickNumbers.map((x) => ({ chapterNumber: x, label: `第 ${x} 章` }));
          if (units.length === 0) {
            const msg = "写后处理需要已写好的章节：请先撰写至少一章。";
            snapshotAutomationExecution(bookId, { running: false, completedCount: 0, failures: 0, stepProgress: undefined, latestError: msg });
            broadcast("auto:error", { bookId, error: msg });
            return;
          }
          snapshotAutomationExecution(bookId, { totalUnits: units.length });
          broadcast("auto:snapshot", automationExecutions.get(bookId) ?? {});
          const pwResults: Array<{ chapterNumber: number; status: "reviewed" | "error" }> = [];
          for (let ui = 0; ui < units.length; ui++) {
            const { chapterNumber, label } = units[ui];
            if (skipLabels.has(label)) continue;
            if (abortController.signal.aborted) {
              snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
              broadcast("auto:paused", { bookId, completedCount: pwResults.length, totalUnits: units.length });
              return;
            }
            snapshotAutomationExecution(bookId, { currentUnitLabel: label, currentChapterNumber: chapterNumber });
            let unitFailed = false;
            for (let si = 0; si < enabledSteps.length; si++) {
              ...
              broadcastStepStart(bookId, chapterNumber, si, enabledSteps.length, step.agent);
              try {
                // Post-write agents: length-normalizer, polisher, consolidator.
                // Others are informational no-ops with a "done" broadcast.
                if (step.agent === "consolidator") {
                  const agent = new ConsolidatorAgent(pipeline.createAgentContext("consolidator", bookId));
                  await agent.consolidate(bookDir).catch(() => ({ merged: [] }));
                } else {
                  void step.agent;
            ...
            const unitStatus: "reviewed" | "error" = unitFailed ? "error" : "reviewed";
            pwResults.push({ chapterNumber, status: unitStatus });
            // Post-write pass does not run auditors, so it cannot judge
            // pass/fail. Only persist a status change when a step failed
            // (mark audit-failed so the user sees something went wrong); on
            // success leave the existing status untouched.
            if (unitFailed) {
              await persistChapterReviewOutcome(chapterNumber, [], true);
            }
            const completedLabels = [...(automationExecutions.get(bookId)?.completedUnitLabels ?? []), label];
            snapshotAutomationExecution(bookId, { completedUnitLabels: completedLabels });
            try {
              broadcast("auto:chapter-complete", {
                bookId, chapterNumber, status: unitStatus,
                completedCount: pwResults.filter((r) => r.status === "reviewed").length,
                requestedCount: units.length,
              });
            } catch { /* best-effort */ }
          }
          const failures = pwResults.filter((r) => r.status === "error").length;
          snapshotAutomationExecution(bookId, { running: false, paused: false, completedCount: pwResults.length, failures, stepProgress: undefined });
          broadcast("auto:complete", { bookId, totalCompleted: pwResults.length, failures, workflowId: wf.id });
        } else if (wf.target === "market-radar") {
          // Radar is a single-unit scan: one step that runs the radar agent
          // and persists the result to the workspace radar/ directory.
          if (abortController.signal.aborted) {
            snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
            broadcast("auto:paused", { bookId, completedCount: 0, totalUnits: 1 });
            return;
          }
          snapshotAutomationExecution(bookId, { totalUnits: 1, currentUnitLabel: "市场雷达扫描", currentChapterNumber: undefined });
          for (let si = 0; si < enabledSteps.length; si++) {
            if (abortController.signal.aborted) {
              snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
              broadcast("auto:paused", { bookId, completedCount: 0, totalUnits: 1 });
            ...
            const step = enabledSteps[si];
            broadcastStepStart(bookId, 1, si, enabledSteps.length, step.agent);
            try {
              if (step.agent === "radar") {
                const result = await pipeline.runRadar();
                await saveRadarScan(root, result).catch(() => {});
              } else {
                void step.agent;
              }
              broadcastStepDone(bookId, 1, si, enabledSteps.length, step.agent, "done");
            } catch (e) {
              broadcastStepDone(bookId, 1, si, enabledSteps.length, step.agent, "error");
              const message = e instanceof Error ? e.message : String(e);
              snapshotAutomationExecution(bookId, { running: false, completedCount: 0, failures: 1, stepProgress: undefined, latestError: message });
              broadcast("auto:error", { bookId, error: message });
              return;
            }
          }
          snapshotAutomationExecution(bookId, { running: false, paused: false, completedCount: 1, failures: 0, stepProgress: undefined });
          broadcast("auto:chapter-complete", { bookId, chapterNumber: 1, status: "reviewed", completedCount: 1, requestedCount: 1 });
          broadcast("auto:complete", { bookId, totalCompleted: 1, failures: 0, workflowId: wf.id });
        }
      } catch (e) {
        const message = e instanceof Error ? e.message : String(e);
        snapshotAutomationExecution(bookId, { running: false, latestError: message });
        try { broadcast("auto:error", { bookId, error: message }); } catch { /* best-effort */ }
      } finally {
        automationControllers.delete(bookId);
      }
    })().catch(() => { /* fire-and-forget, errors broadcast above */ });

    return c.json({ status: "workflow-running", bookId, workflowId: wf.id, workflowName: wf.name, target: wf.target, chapterCount: totalUnits, scope, skills: requestedSkills ?? wf.skills });
  });

  /**
   * 终止自动化写作。触发 AbortController.abort()，执行循环在下一个检查点
   * 捕获到 aborted 信号后暂停，保留已完成单元的快照（paused=true），
   * 可通过 POST /resume 继续。
   */
  app.post("/api/v1/books/:id/automation/abort", async (c) => {
    const bookId = c.req.param("id");
    const controller = automationControllers.get(bookId);
    if (!controller) return c.json({ error: "no running automation" }, 404);
    controller.abort();
    // Mark the snapshot as paused immediately so the frontend can reflect the
    // abort state without waiting for the in-flight step to observe the signal.
    // The workflow loop will still broadcast auto:paused with accurate counts
    // once it unwinds; this just prevents the UI from looking stuck.
    snapshotAutomationExecution(bookId, { running: false, paused: true, stepProgress: undefined });
    return c.json({ ok: true, bookId, message: "automation aborting" });
  });

  /**
   * 继续已暂停的自动化写作。重新发起与原运行相同的工作流，
   * 传入 isResume 标志让 run handler 跳过已完成的单元。
   */
  app.post("/api/v1/books/:id/automation/resume", async (c) => {
    const bookId = c.req.param("id");
    const snap = automationExecutions.get(bookId);
    if (!snap) return c.json({ error: "no paused execution" }, 404);
    if (!snap.paused) return c.json({ error: "execution is not paused" }, 400);

    // Re-invoke the workflow run endpoint logic by forwarding to the same handler.
    // We do a server-side redirect via fetch to keep the code DRY.
    const resumeBody: Record<string, unknown> = {
      chapterCount: snap.resumeContext?.chapterCount ?? snap.totalUnits,
    };
    if (snap.scope) resumeBody.scope = snap.scope;
    if (snap.resumeContext?.skills) resumeBody.skills = snap.resumeContext.skills;

    const workflowId = snap.workflowId;
    // NOTE: Do NOT clear paused here — the run handler checks prevSnapshot.paused
    // to decide whether to skip already-completed units. It will clear it.

    // Re-dispatch to the run handler by calling the same internal path.
    const runUrl = new URL(c.req.url);
    runUrl.pathname = `/api/v1/books/${encodeURIComponent(bookId)}/workflow/${encodeURIComponent(workflowId)}/run`;
    try {
      ...
      broadcast("auto:resumed", { bookId, workflowId });
      return c.json(data);
    } catch (e) {
      return c.json({ error: e instanceof Error ? e.message : "resume failed" }, 500);
    }
  });

  /**
   * Return the latest automation execution snapshot for a book (or all
   * executions when no bookId is supplied). Used by the AutomationPage
   * frontend to restore progress / step state / log tail when the user
   * navigates away then back to the page.
   */
  app.get("/api/v1/executions/current", async (c) => {
    const bookId = c.req.query("bookId");
    if (bookId) {
      const snap = automationExecutions.get(bookId);
      return c.json(snap ? { execution: snap } : { execution: null });
    }
    const all = Array.from(automationExecutions.values());
  ...
  app.post("/api/v1/books/:id/unlock", async (c) => {
    ...
    const id = normalizeApiBookId(rawId, "id") ?? rawId;
    try {
      const result = await state.forceUnlockBook(id);
      return c.json(result);
    } catch (e) {
      return c.json({ cleared: false, reason: String(e) }, 500);
    }
  });
  ...
  app.get("/api/v1/events", (c) => {
    return streamSSE(c, async (stream) => {
      ...
      // Restore latest automation execution for this book so users can
      // navigate away from the Automation page and return without losing
      // the progress bar, step state, and unit label.
      const bookIdForExec = c.req.query("bookId");
      if (bookIdForExec) {
        const exec = automationExecutions.get(bookIdForExec);
        if (exec) await stream.writeSSE({ event: "auto:snapshot", data: JSON.stringify(exec) });
      }
  ...
  app.get("/api/v1/studio/global-config", async (c) => {
    const config: StudioGlobalConfig = await loadStudioGlobalConfig();
    return c.json(config);
  });

  app.put("/api/v1/studio/global-config", async (c) => {
    const body = (await c.req.json()) as Partial<StudioGlobalConfig>;
    const allowed: Partial<Omit<StudioGlobalConfig, "version" | "updatedAt">> = {};
    if (typeof body.lastProjectRoot === "string") {
      allowed.lastProjectRoot = body.lastProjectRoot;
    }
    const config = await saveStudioGlobalConfig(allowed);
    return c.json(config);
  });
  ...
  app.post("/api/v1/skills/import", async (c) => {
    ...
    const result = await importStudioSkillFolder(root, payload);
    const refreshed = await loadStudioSkills(root);
    if (result.kind === "standalone") {
      const skill = refreshed.skills.find((s) => s.id === result.skill.id);
      if (!skill) throw new ApiError(500, "INTERNAL_ERROR", "Skill import succeeded but skill not found after refresh");
      return c.json({ kind: "standalone", skill });
    }
    const pkg = refreshed.packages.find((p) => p.id === result.packageId);
    if (!pkg) throw new ApiError(500, "INTERNAL_ERROR", "Skill package import succeeded but package not found after refresh");
    const children = refreshed.skills.filter((s) => pkg.skillIds.includes(s.id));
    return c.json({ kind: "package", package: pkg, skills: children });
  });

  app.delete("/api/v1/skills/:skillId", async (c) => {
    const paramId = normalizeStudioSkillId(c.req.param("skillId"), "skillId");
    const projectDir = projectSkillDir(root, paramId);
    try {
      const statResult = await stat(projectDir);
      if (!statResult.isDirectory()) {
        throw new ApiError(404, "SKILL_NOT_FOUND", `Project skill not found: ${paramId}`);
      }
    } catch (error) {
    ...
    // 判断是单 skill 还是 package 目录：
    let targetIsPackage = false;
    try {
      const manifest = await stat(projectSkillPath(root, paramId));
      if (manifest.isFile()) {
        targetIsPackage = false;
      }
    } catch {
      targetIsPackage = true;
    }
    await rm(projectDir, { recursive: true, force: true });
    return c.json({ ok: true, kind: targetIsPackage ? "package" : "standalone", id: paramId });
  });
  ...
  app.get("/api/v1/books/:id/truth", async (c) => {
    ...
    const { isNewLayoutBook } = await import("@inkrhyme/core");
  ...
  app.post("/api/v1/books/:id/audit/:chapter", async (c) => {
    ...
    try {
      ...
      const content = await readFile(join(chaptersDir, match), "utf-8");
      const currentConfig = await loadCurrentProjectConfig();
      const { ContinuityAuditor } = await import("@inkrhyme/core");
  ...
  app.get("/api/v1/genres/:id", async (c) => {
    ...
    try {
      const { readGenreProfile } = await import("@inkrhyme/core");
  ...
  app.post("/api/v1/genres/:id/copy", async (c) => {
    ...
    try {
      const { getBuiltinGenresDir } = await import("@inkrhyme/core");
  ...
  app.post("/api/v1/books/:id/detect/:chapter", async (c) => {
    ...
    try {
      ...
      const content = await readFile(join(chaptersDir, match), "utf-8");
      const { analyzeAITells } = await import("@inkrhyme/core");
  ...
  app.get("/api/v1/books/:id/detect/stats", async (c) => {
    const id = c.req.param("id");
    try {
      const { loadDetectionHistory, analyzeDetectionInsights } = await import("@inkrhyme/core");
      const bookDir = state.bookDir(id);
  ...
  app.post("/api/v1/style/analyze", async (c) => {
    ...
    try {
      const { analyzeStyle } = await import("@inkrhyme/core");
  ...
  app.post("/api/v1/books/:id/import/chapters", async (c) => {
    ...
    try {
      const { splitChapters } = await import("@inkrhyme/core");
  ...
  app.get("/api/v1/doctor", async (c) => {
    ...
    const { GLOBAL_ENV_PATH } = await import("@inkrhyme/core");
