...
vi.mock("@inkrhyme/core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@inkrhyme/core")>();
...
describe("createStudioServer daemon lifecycle", () => {
  ...
  it("uses the real core bookId validator in the Studio safety mock", async () => {
    const { isSafeBookId } = await import("@inkrhyme/core");
  ...
  async function wireRealSessionTranscript() {
    const actual = await vi.importActual<typeof import("@inkrhyme/core")>("@inkrhyme/core");
  ...
  it("tags task pipeline log broadcasts with the execution id while chat round logs stay untagged", async () => {
    ...
    const core = await import("@inkrhyme/core");
    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
