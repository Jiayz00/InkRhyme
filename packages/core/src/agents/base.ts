import type { LLMClient, LLMMessage, LLMResponse, OnStreamProgress, ResponseFormat } from "../llm/provider.js";
...
export interface AgentContext {
  ...
  /** 已解析的 Skill 指导文本，由 PipelineRunner 从 SkillRegistry 生成并透传 */
...
export abstract class BaseAgent {
  ...
  protected async chat(
    ...
    options?: { readonly temperature?: number; readonly maxTokens?: number; readonly responseFormat?: ResponseFormat },
  ): Promise<LLMResponse> {
    const finalMessages = this.ctx.skillGuidance
      ? PipelineRunnerSkillInjector.inject(messages, this.ctx.skillGuidance)
      : messages;
...
/**
 * 将 skillGuidance 注入到 messages 的第一条 system 消息中。
 * 如果没有 system 消息，则在最前面插入一条。
 * 独立为类方法方便测试，且不污染 BaseAgent 的 protected 接口。
 */
export const PipelineRunnerSkillInjector = {
  inject(messages: ReadonlyArray<LLMMessage>, skillGuidance: string): LLMMessage[] {
    ...
    return [{ role: "system" as const, content: skillGuidance }, ...messages];
  },
};
