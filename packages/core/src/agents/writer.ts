...
import { cachedReadFile } from "../utils/book-context-cache.js";
import {
...
export class WriterAgent extends BaseAgent {
  ...
  private async settle(params: {
    ...
    // 合并路径：跳过 Observer，Settler 直接从正文 + truths 原文开始一次调用完成"提取+合并"
    ...
    // 解析响应：先尝试 delta，再尝试 legacy settlement + governed merge
    ...
    try {
      ...
      // 检测解析失败：响应里完全没有 POST_SETTLEMENT 和 CHAPTER_SUMMARY 标签，说明不是合法 settler 输出
      if (!settlement.postSettlement && !settlement.chapterSummary) {
        parseFailed = true;
      }
    ...
    // 合并路径解析失败 → fallback 回原两步路径（Observer + Settler）
    if (parseFailed) {
      return this.settleWithObserver(params);
    }
  ...
  // 原两步路径：Phase 2a Observer 提取事实 + Phase 2b Settler 合并到 truths
  // 合并路径解析失败时 fallback 回到这里
  private async settleWithObserver(params: {
    readonly book: BookConfig;
    readonly genreProfile: GenreProfile;
    readonly bookRules: BookRules | null;
    readonly chapterNumber: number;
  ...
  private async loadRecentChapters(
    ...
    try {
      ...
      const contents = await Promise.all(
        mdFiles.map(async (f) => {
          const content = await cachedReadFile(join(chaptersDir, f));
          return content;
        }),
      );
