...
输出一个 JSON 对象。不要包裹在 Markdown 代码块中，不要添加任何前后说明文字。结构如下：

{
  "goal": "本章目标（不超过 50 字）",
  "threadRefs": ["H03", "S004"],
  "currentTask": "一句话：本章主角要完成的具体动作，不要抽象描述",
  "readerExpectation": "两行：\\n1) 读者现在期待什么（基于前几章的埋伏）\\n2) 本章对这个期待做什么——制造更强缺口 / 部分兑现 / 完全兑现 / 暂不兑现但给暗示",
  "payoff": "- 该兑现：X → 兑现到什么程度\\n- 暂不掀：Y → 先压住，留到第 N 章",
  "transitionalBeats": "如果本章是非高压章节，每段非冲突段落说明功能。格式：[段落位置] → [承担功能]\\n如果本章是高压/冲突章节，写'不适用 - 本章无日常过渡'",
  "keyChoice": "- 主角本章最关键的一次选择：\\n  - 为什么这么做？\\n  - 符合当前利益吗？\\n  - 符合他的人设吗？\\n- 对手/配角本章最关键的一次选择：\\n  - 为什么这么做？\\n  - 符合当前利益吗？\\n  - 符合他的人设吗？",
  "endChange": "1-3 条，从以下维度选：信息改变 / 关系改变 / 物理改变 / 权力改变",
  "hookLedger": "open:\\n- [new] 新钩子描述（<=30字）|| 理由\\n\\nadvance:\\n- H007 '胖虎借条' → 林秋第一次想撕，被阻止（planted → pressured）\\n\\nresolve:\\n- H003 '杂役腰牌' → 林秋主动摘下（clear）\\n\\ndefer:\\n- H009 '守拙诀来历' → 本章不动，理由：时机不到",
  "doNot": "2-4 条硬约束；没有则写'无'"
}

各字段说明：
- goal: 不超过 50 字的具体目标
- threadRefs: 从输入 pending_hooks/subplot_board 中挑出的 id 数组；没有就空数组 []
- currentTask: 一句话，本章主角要完成的具体动作，不要抽象描述
- readerExpectation: 两行内容——1) 读者现在期待什么 2) 本章对期待做什么
- payoff: 该兑现的与暂不掀的，每条一行
- transitionalBeats: 日常/过渡承担什么任务；高压章节写"不适用 - 本章无日常过渡"
- keyChoice: 主角和对手/配角各自最关键的一次选择，以及三连问回答
- endChange: 1-3 条章尾必须发生的改变
- hookLedger: 按 open/advance/resolve/defer 分类列出本章 hook 操作
- doNot: 2-4 条硬约束；没有额外禁止事项就写"无"

### hookLedger 格式

hookLedger 是一个字符串字段，内含按分类组织的 hook 操作列表。格式（用 \\n 换行）：
...
- 本章"currentTask"如果天然对应某个 hook 的兑现动作，必须在 resolve 里显式声明对应 hook_id

## 输出要求

- goal 不超过 50 字
- threadRefs 用数组写从输入 pending_hooks/subplot_board 中挑出的 id；没有就空数组 []
- 每个字段不能为空字符串（doNot 允许写"无"）
...
Output a single JSON object. Do NOT wrap it in a Markdown code block. Do NOT add any prose before or after the JSON. Structure:

{
  "goal": "Chapter goal (max 50 chars)",
  "threadRefs": ["H03", "S004"],
  "currentTask": "One sentence: the concrete action the protagonist must complete this chapter — no abstractions",
  "readerExpectation": "Two lines:\\n1) what the reader currently expects (based on prior chapters' setups)\\n2) what this chapter does with that expectation — widen the gap / partial payoff / full payoff / hint without paying off",
  "payoff": "- Pay off: X → to what degree\\n- Keep buried: Y → suppress until chapter N",
  "transitionalBeats": "if non-pressure chapter, name each non-conflict paragraph's function: [position] → [function]\\nif pressure / conflict chapter, write 'n/a — pressure chapter, no transitional beats'",
  "keyChoice": "- Protagonist's most important choice:\\n  - Why this choice?\\n  - Does it match current interest?\\n  - Does it match their persona?\\n- Antagonist / supporting cast's most important choice:\\n  - Why this choice?\\n  - Does it match current interest?\\n  - Does it match their persona?",
  "endChange": "1-3 items, choose from: information change / relationship change / physical change / power change",
  "hookLedger": "open:\\n- [new] new hook description (<=30 chars) || reason\\n\\nadvance:\\n- H007 'Huzi IOU' → Lin Qiu tries to tear it, gets stopped (planted → pressured)\\n\\nresolve:\\n- H003 'errand badge' → Lin Qiu unpins it himself (clear)\\n\\ndefer:\\n- H009 'origin of Shou-Zhuo Jue' → not touched, reason: timing not right",
  "doNot": "2-4 hard prohibitions; if none, write 'none'"
}

Field descriptions:
- goal: max 50 chars, the concrete chapter goal
- threadRefs: array of ids picked from input pending_hooks / subplot_board; empty array [] if none
- currentTask: one sentence, the concrete action the protagonist must complete
- readerExpectation: two lines — 1) what reader expects 2) what this chapter does with it
- payoff: pay off / keep buried items, one per line
- transitionalBeats: function of slow/transitional beats; pressure chapters write "n/a — pressure chapter, no transitional beats"
- keyChoice: protagonist's and antagonist's key choices with three-question check
- endChange: 1-3 required end-of-chapter changes
- hookLedger: open/advance/resolve/defer categorized hook operations
- doNot: 2-4 hard prohibitions; "none" if no extra prohibitions

### hookLedger format

hookLedger is a string field containing categorized hook operations. Format (use \\n for line breaks):
...
- If "currentTask" naturally corresponds to paying off a hook, it must appear under resolve with the hook_id.

## Output requirements

- goal is max 50 characters
- threadRefs is an array of ids picked from input pending_hooks / subplot_board; empty array [] if none
- Every field must be a non-empty string (doNot allows "none")
...
Produce the memo for chapter {{chapterNumber}}. Strictly emit the JSON object format above, do not wrap it in a code block.`;
...
请为第 {{chapterNumber}} 章产生 memo。严格按上面的 JSON 对象格式输出，不要包裹在代码块中。`;
