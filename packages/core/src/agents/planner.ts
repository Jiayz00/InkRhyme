...
import { parseMemo, parseMemoJson, PlannerParseError } from "../utils/chapter-memo-parser.js";
import {
 ...
 *   - a full ChapterMemo via LLM call using JSON structured output.
 *
 * JSON mode policy: the LLM is asked to emit a JSON object matching
 * PlannerMemoJsonSchema. If parsing fails (rare with JSON mode), the planner
 * emits a degraded but valid fallback memo with an explicit warning instead of
 * crashing the whole chapter pipeline. No retry loop — JSON mode makes
 * syntax failures impossible, and semantic failures are rare enough that the
...
export class PlannerAgent extends BaseAgent {
  ...
  /**
   * Invoke the LLM to produce a structured JSON memo and parse it.
   *
   * Uses JSON mode (response_format: json_object) to guarantee valid JSON
   * syntax. If parsing or schema validation fails, falls back to a degraded
   * but valid memo — no retry loop needed.
   */
  async planChapterMemo(input: {
    ...
    try {
      return parseMemoJson(response.content, input.chapterNumber, input.isGoldenOpening, language);
    } catch (error) {
      if (!(error instanceof PlannerParseError) && !(error instanceof Error)) {
        throw error;
      }
      const errorMessage = error instanceof PlannerParseError
        ? error.message
        : `schema validation failed: ${error.message}`;
      this.log?.warn(`[planner] JSON memo parse failed, using fallback: ${errorMessage}`);
      return parseMemo(
  ...
  private buildFallbackMemoMarkdown(input: {
    ...
    if (input.language === "en") {
      return [
        ...
        "",
        "## Planner warning",
        `The model failed to produce a valid chapter memo (JSON parse error). Last parser error: ${input.errorMessage}`,
      ].join("\n");
    }
    ...
    return [
      ...
      "",
      "## Planner warning",
      `模型未能产出合格章节 memo（JSON 解析错误）。最后一次解析错误：${input.errorMessage}`,
    ].join("\n");
  }
