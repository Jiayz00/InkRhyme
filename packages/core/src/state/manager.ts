...
const BOOK_LOCK_HEARTBEAT_MS = 15_000;
const BOOK_LOCK_LEASE_MS = 2 * 60_000;
...
export class StateManager {
  ...
  async ensureControlDocumentsAt(
    ...
    // Cheap, deterministic seeds -> all stat/write parallel.
    await Promise.all([
      this.writeIfMissing(
    ...
    // style_guide: only touch disk onces we know for sure we need to write.
    const styleGuidePath = join(storyDir, "style_guide.md");
    let existing: string | undefined;
    try {
      existing = await readFile(styleGuidePath, "utf-8");
    } catch {
      // file missing — build full seed content once below.
    }
    ...
    const markerZh = "写作方法论";
    const markerEn = "Writing Methodology";
    if (!existing.includes(markerZh) && !existing.includes(markerEn)) {
      const { buildWritingMethodologySection } = await import("../utils/writing-methodology.js");
      // Avoid writeFile-if-empty races with two callers: only append when the
      // marker is genuinely absent AND the file wasn't already expanded.
  ...
  async acquireBookLock(bookId: string): Promise<() => Promise<void>> {
    ...
    try {
      ...
      return async () => {
        ...
        // Fire-and-forget: don't wait for the heartbeat task to flush.
        // The heartbeat is a background timer; the next acquire detects
        // staleness via timestamp comparison. Waiting here adds up to 2s
        // per chapter in batch writing scenarios.
        owner.heartbeatTask?.catch(() => void 0);
  ...
  async forceUnlockBook(bookId: string): Promise<{ cleared: boolean; reason?: string }> {
    await mkdir(this.bookDir(bookId), { recursive: true });
    const lockPath = join(this.bookDir(bookId), ".write.lock");
    const lockKey = this.normalizeLockKey(lockPath);
    ...
    const inProcessEntry = processBookLocks.get(lockKey);
    if (inProcessEntry) {
      if (inProcessEntry.heartbeatTimer) clearInterval(inProcessEntry.heartbeatTimer);
      processBookLocks.delete(lockKey);
      cleared = true;
    }
    ...
    try {
      ...
      const code = (error as NodeJS.ErrnoException | undefined)?.code;
      if (code === "ENOENT") {
        if (!cleared) return { cleared: false, reason: "No lock found" };
      }
    }
  ...
  private async rebuildChapterIndexFromFilesAt(bookDir: string): Promise<ReadonlyArray<ChapterMeta>> {
    ...
    const rows = await Promise.all(files.flatMap(async (file) => {
      // Accept both "0001.md" / "0001_title.md" and the "ch0001.md" naming
      // convention used by some existing projects.
      const match = file.match(/^(?:ch)?(\d+)[_-]?(.*?)\.md$/i);
      if (!match) return [];
  ...
  async saveChapterIndexAt(
    ...
    // Fast path: explicit empty index AND caller said it's safe to ignore any
    // pre-existing chapter files on disk. `rebuildChapterIndexFromFilesAt`
    // does a full dir scan + readFile on every .md under chapters/, which can
    // take tens to hundreds of ms per chapter. During initBook we always want
    // a clean empty index, so skip the scan entirely.
    let safeIndex: ReadonlyArray<ChapterMeta>;
    if (index.length === 0 && options.allowEmptyWithChapterFiles) {
      safeIndex = index;
    } else if (index.length === 0) {
      // Only fall back to the slow scan when we actually need to rebuild:
      // i.e. the chapters directory exists and might have markdown files in
      // it already. Otherwise we'd rebuild from an empty folder and get []
      // anyway — an avoidable stat/readdir roundtrip.
      let scanRequired = false;
      try {
        const dirEntries = await readdir(chaptersDir);
        scanRequired = dirEntries.some((name) => name.endsWith(".md"));
      } catch {
        // ENOENT / access denied — no markdown files to worry about.
        scanRequired = false;
      }
      if (scanRequired) {
        const rebuilt = await this.rebuildChapterIndexFromFilesAt(bookDir);
        safeIndex = rebuilt.length > 0 ? rebuilt : index;
      } else {
        safeIndex = index;
      }
    } else {
