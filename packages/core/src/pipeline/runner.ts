...
// Re-export importer functions for backward compatibility (tests import from runner.js)
export { buildImportFoundationSource, buildSpinoffFoundationContext, resetImportReplayTruthFiles } from "./importer.js";
import { buildImportFoundationSource, buildSpinoffFoundationContext } from "./importer.js";
import { persistChapterArtifacts } from "./chapter-persistence.js";
import {
  prepareWriteInput as builderPrepareWriteInput,
  createGovernedArtifacts as builderCreateGovernedArtifacts,
  resolveGovernedPlan as builderResolveGovernedPlan,
  buildPersistenceOutput as builderBuildPersistenceOutput,
} from "./artifact-builder.js";
import { runChapterReviewCycle } from "./chapter-review-cycle.js";
import {
  createPersistenceState,
  syncCurrentStateFactHistory as helperSyncCurrentStateFactHistory,
  syncLegacyStructuredStateFromMarkdown as helperSyncLegacyStructuredStateFromMarkdown,
  syncNarrativeMemoryIndex as helperSyncNarrativeMemoryIndex,
  persistAuditDriftGuidance as helperPersistAuditDriftGuidance,
  stripAuditDriftCorrectionBlock as helperStripAuditDriftCorrectionBlock,
  markBookActiveIfNeeded as helperMarkBookActiveIfNeeded,
  buildLengthWarnings as helperBuildLengthWarnings,
  buildLengthTelemetry as helperBuildLengthTelemetry,
  logLengthWarnings as helperLogLengthWarnings,
  restoreLostAuditIssues as helperRestoreLostAuditIssues,
  type PersistenceState,
} from "./pipeline-helpers.js";
import { BookContextCache, withBookContextCache } from "../utils/book-context-cache.js";
...
export { BUILTIN_AGENT_NAMES, type BuiltinAgentName } from "../agents/builtin-names.js";
...
export interface PipelineConfig {
  ...
  /** 已解析的 Skill 指导文本，注入到每个子 Agent 的 system prompt */
  readonly skillGuidance?: string;
  /** 自动化写作阶段的实时日志回调，由 server.ts 设置为 SSE 广播 */
  readonly onStageLog?: (stage: string, message: string, detail?: unknown) => void;
}
...
/** Result of prepareWriteInput, cached for pipelining the next chapter. */
type PreparedWriteInput = Pick<
  WriteChapterInput,
  "externalContext" | "chapterIntent" | "chapterMemo" | "chapterIntentData" | "contextPackage" | "ruleStack"
>;

/** A prefetched prepare result paired with the chapter number it was built for. */
interface PrefetchedPrepare {
  readonly result: PreparedWriteInput;
  readonly chapterNumber: number;
}
...
export class PipelineRunner {
  ...
  private readonly persistenceState: PersistenceState;

  constructor(config: PipelineConfig) {
    this.config = config;
    this.state = new StateManager(config.projectRoot);
    this.persistenceState = createPersistenceState();
  }
  ...
  private logStage(language: LengthLanguage, message: { zh: string; en: string }): void {
    const localized = this.localize(language, message);
    this.config.logger?.info(
      `${this.localize(language, { zh: "阶段：", en: "Stage: " })}${localized}`,
    );
    this.config.onStageLog?.("stage", localized);
  }
  ...
  private agentCtxFor(agent: string, bookId?: string): AgentContext {
    ...
    return {
      ...
      skillGuidance: this.config.skillGuidance,
    };
  }
  ...
  async writeNextChapter(bookId: string, wordCount?: number, temperatureOverride?: number): Promise<ChapterPipelineResult> {
    ...
    const releaseLock = await this.state.acquireBookLock(bookId);
    try {
      const cache = new BookContextCache();
      return await withBookContextCache(cache, () => this._writeNextChapterLocked(
        bookId, wordCount, temperatureOverride, this.config.externalContext, { contextCache: cache },
      ));
    } finally {
      await releaseLock();
    }
  ...
  async writeChapters(
    ...
    // Prefetched prepare for the next chapter — started during the current
    // chapter's validation phase so the planner LLM call overlaps with
    // validation instead of running serially after it.
    let nextPreparePromise: Promise<PrefetchedPrepare | null> | null = null;
    // BookContextCache populated by the prefetched prepare, reused when the
    // same chapter's writeChapter runs so the writer hits the cache instead of
    // re-reading every truth file that planner + composer already loaded.
    let nextCache: BookContextCache | null = null;

    for (let index = 0; index < chapterCount; index += 1) {
      this.throwIfOperationAborted();
      const releaseLock = await this.state.acquireBookLock(bookId);
      let prefetchedPrepare: PrefetchedPrepare | null = null;
      const cache = nextCache ?? new BookContextCache();
      nextCache = null;
      try {
        // Await any prefetched prepare from the previous iteration.
        if (nextPreparePromise) {
          prefetchedPrepare = await nextPreparePromise;
          nextPreparePromise = null;
        }

        const result = await withBookContextCache(cache, () => this._writeNextChapterLocked(
          bookId,
          options.wordCount,
          ...
          this.config.externalContext,
          {
            prefetchedPrepare,
            contextCache: cache,
            onEarlyPersist: ({ book, bookDir, chapterNumber }) => {
              // Kick off next chapter's prepare (planner + composer) without
              // holding the book lock. It reads truths already persisted to
              // disk and writes only per-chapter runtime files, so it cannot
              // conflict with the current chapter's final persist. If it
              // fails (LLM error, parse error), it resolves to null and the
              // next iteration falls back to a normal prepareWriteInput.
              const nextChapterNumber = chapterNumber + 1;
              // Fresh cache for the next chapter — its truths differ from the
              // current chapter's (early-persist just wrote updated state).
              // The same cache is handed to the next iteration's
              // _writeNextChapterLocked so writeChapter reuses the prepare's
              // reads.
              const freshCache = new BookContextCache();
              nextCache = freshCache;
              nextPreparePromise = withBookContextCache(freshCache, () =>
                ...
                .then((result) => ({ result, chapterNumber: nextChapterNumber }))
                .catch(() => null);
            },
          },
        ));
        ...
        results.push({
          ...
          auditResult: {
            passed: false,
            parseFailed: true,
            issues: [{ category: "PipelineError", severity: "critical", description: String(error), suggestion: "Check the logs and retry." }],
            summary: `Chapter pipeline failed: ${String(error)}`,
          },
        });
      } finally {
        await releaseLock();
      }
      options.onChapterComplete?.(results[results.length - 1]!, results.length, chapterCount);
      const lastStatus = results[results.length - 1]!.status;
      if (lastStatus !== "ready-for-review" && lastStatus !== "audit-failed") {
        // state-degraded or other terminal status — discard any prefetched
        // prepare (it used pre-retry truths) and stop the batch.
        nextPreparePromise = null;
        nextCache = null;
        break;
      }
  ...
  private async _writeNextChapterLocked(
    ...
    options?: {
      readonly prefetchedPrepare?: PrefetchedPrepare | null;
      readonly contextCache?: BookContextCache;
      readonly onEarlyPersist?: (info: {
    ...
    const stageLanguage = await this.resolveBookLanguage(book);
    this.logStage(stageLanguage, { zh: "准备章节输入", en: "preparing chapter inputs" });
    const writeInput = options?.prefetchedPrepare && options.prefetchedPrepare.chapterNumber === chapterNumber
      ? options.prefetchedPrepare.result
      : await this.prepareWriteInput(book, bookDir, chapterNumber, externalContext);
    ...
    this.throwIfOperationAborted();
    // Drop every cached truth-file read now that writeChapter has returned.
    // Post-write phases (hook promotion, early-persist, validation retry via
    // settleChapterState) mutate and re-read truth files, so they must hit
    // disk directly instead of seeing the pre-write snapshot the planner /
    // composer / writer shared above.
    options?.contextCache?.clear();
    const writerCount = countChapterLength(output.content, lengthSpec.countingMode);
    ...
    if ((this.config.chapterReviewMode ?? "auto") === "manual") {
      ...
      // Wrap the review cycle in a fresh BookContextCache so auditor and
      // reviser share memoized truth-file reads. The pre-write cache was
      // cleared above (line ~1952) because settler mutated truth files;
      // this new cache starts fresh and covers only the read-only audit
      // and revise phases, which read the same truth files repeatedly.
      const reviewCache = new BookContextCache();
      const auditor = new ContinuityAuditor(this.agentCtxFor("auditor", bookId));
      const reviewResult = await withBookContextCache(reviewCache, () => runChapterReviewCycle({
    ...
    // 4.1 Capture old truths (before early persist) for validation diff
    ...
    // 4.1a Early persist: write chapter + core truths to disk now so the next
    // chapter's planner/composer can read them while validation runs in
    // parallel. saveChapter is atomic (chapter file + core truths via
    // commitAtomicFileSet). Memory DB is synced so retrieveMemorySelection
    // sees the latest hooks/summaries.
    this.logStage(stageLanguage, { zh: "提前落盘章节与核心真相", en: "early-persisting chapter and core truths" });
    ...
    await this.syncNarrativeMemoryIndex(bookId);
    const earlyPersistenceOutput = persistenceOutput;
    // Signal the caller to kick off the next chapter's prepare (planner +
    // composer) — it reads the freshly-written truths from disk while we
    // validate below.
    options?.onEarlyPersist?.({ book, bookDir, chapterNumber });
    ...
    // 4.1b If validation retry produced different truths, re-persist core
    // truths so disk reflects the corrected settlement. (The prefetched
    // next-chapter prepare — if any — used the pre-retry truths and will be
    // discarded by the caller when it detects a non-ready status.)
    if (persistenceOutput !== earlyPersistenceOutput) {
      this.logStage(stageLanguage, { zh: "重试结算后重新落盘", en: "re-persisting after settlement retry" });
      await writer.saveChapter(bookDir, persistenceOutput, gp.numericalSystem, pipelineLang);
      await this.syncLegacyStructuredStateFromMarkdown(bookDir, chapterNumber, persistenceOutput);
      await this.syncNarrativeMemoryIndex(bookId);
    }
    ...
    await persistChapterArtifacts({
      ...
      // saveChapter already ran in the early-persist phase (4.1a) — skip the
      // redundant atomic write. If retry changed persistenceOutput, it was
      // re-persisted in 4.1b.
      saveChapter: async () => {},
...
${matrix}`,
  ...
  private async buildPersistenceOutput(
    ...
    return builderBuildPersistenceOutput(bookId, book, bookDir, chapterNumber, output, finalContent, countingMode, reducedControlInput, {
      inputGovernanceMode: this.config.inputGovernanceMode,
      onContextCompression: this.config.onContextCompression,
      agentCtxFor: (agent, bookId) => this.agentCtxFor(agent, bookId),
    });
  }
  ...
  private async prepareWriteInput(
    ...
    return builderPrepareWriteInput(book, bookDir, chapterNumber, externalContext, {
      inputGovernanceMode: this.config.inputGovernanceMode,
      onContextCompression: this.config.onContextCompression,
      agentCtxFor: (agent, bookId) => this.agentCtxFor(agent, bookId),
    });
  ...
  private async resetImportReplayTruthFiles(
    ...
    const { resetImportReplayTruthFiles: importerReset } = await import("./importer.js");
    await importerReset(bookDir, language);
  }
  ...
  private async syncCurrentStateFactHistory(bookId: string, uptoChapter: number): Promise<void> {
    const bookDir = this.state.bookDir(bookId);
    await helperSyncCurrentStateFactHistory(bookId, bookDir, uptoChapter, this.persistenceState, {
      logger: this.config.logger,
      localize: (lang, msg) => this.localize(lang, msg),
      resolveBookLanguageById: (id) => this.resolveBookLanguageById(id),
      languageFromLengthSpec: (spec) => this.languageFromLengthSpec(spec),
    });
  }
  ...
  private async syncLegacyStructuredStateFromMarkdown(
    ...
    await helperSyncLegacyStructuredStateFromMarkdown(bookDir, chapterNumber, output);
  }

  private async syncNarrativeMemoryIndex(bookId: string): Promise<void> {
    const bookDir = this.state.bookDir(bookId);
    await helperSyncNarrativeMemoryIndex(bookId, bookDir, this.persistenceState, {
      logger: this.config.logger,
      localize: (lang, msg) => this.localize(lang, msg),
      resolveBookLanguageById: (id) => this.resolveBookLanguageById(id),
      languageFromLengthSpec: (spec) => this.languageFromLengthSpec(spec),
  ...
  // ── Delegated to pipeline-helpers.ts ──
  // The following methods were extracted to pipeline-helpers.ts as pure functions.
  // PipelineRunner delegates to them, passing state/config via params.
  ...
  private buildLengthWarnings(
    ...
    return helperBuildLengthWarnings(chapterNumber, finalCount, lengthSpec,
      (lang, msg) => this.localize(lang, msg),
      (spec) => this.languageFromLengthSpec(spec));
  }
  ...
  private buildLengthTelemetry(params: {
    ...
    return helperBuildLengthTelemetry(params);
  }
  ...
  private async persistAuditDriftGuidance(params: {
    ...
    await helperPersistAuditDriftGuidance({
      ...params,
      localize: (lang, msg) => this.localize(lang, msg),
    });
  }
  ...
  private stripAuditDriftCorrectionBlock(currentState: string): string {
    return helperStripAuditDriftCorrectionBlock(currentState);
  }

  private logLengthWarnings(lengthWarnings: ReadonlyArray<string>): void {
    helperLogLengthWarnings(lengthWarnings, this.config.logger);
  }

  private restoreLostAuditIssues(previous: AuditResult, next: AuditResult): AuditResult {
    return helperRestoreLostAuditIssues(previous, next);
  }
  ...
  private async markBookActiveIfNeeded(bookId: string): Promise<void> {
    await helperMarkBookActiveIfNeeded(bookId, {
      loadBookConfig: (id) => this.state.loadBookConfig(id),
      saveBookConfig: (id, config) => this.state.saveBookConfig(id, config as BookConfig),
    });
  }
  ...
  private async createGovernedArtifacts(
    ...
    return builderCreateGovernedArtifacts(book, bookDir, chapterNumber, externalContext, {
      inputGovernanceMode: this.config.inputGovernanceMode,
      onContextCompression: this.config.onContextCompression,
      agentCtxFor: (agent, bookId) => this.agentCtxFor(agent, bookId),
    }, options);
  ...
  private async resolveGovernedPlan(
    ...
    return builderResolveGovernedPlan(book, bookDir, chapterNumber, externalContext, {
      inputGovernanceMode: this.config.inputGovernanceMode,
      onContextCompression: this.config.onContextCompression,
      agentCtxFor: (agent, bookId) => this.agentCtxFor(agent, bookId),
    }, options);
