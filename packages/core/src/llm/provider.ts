...
/**
 * 流式 reasoning 回调（思考内容 / Reasoning / Chain-of-Thought 折叠段）。
 * 顺序契约：onThinkingStart → 0 次或多次 onThinkingDelta → 最多一次 onThinkingEnd。
 * 如果上游是 reasoning_content 模式（只能拿到累积的 reasoning 文本而没有开始/结束信号），
 * 实现方会在首次非空 delta 时补发 onThinkingStart，在整条流结束时补发 onThinkingEnd。
 */
export interface OnThinkingCallbacks {
...
async function chatCompletionViaCustomAnthropicCompatible(
  ...
  resolved: { readonly temperature: number; readonly maxTokens: number; readonly extra: Record<string, unknown>; readonly responseFormat?: ResponseFormat },
  onStreamProgress?: OnStreamProgress,
  onTextDelta?: (text: string) => void,
  thinking?: OnThinkingCallbacks,
  signal?: AbortSignal,
): Promise<LLMResponse> {
...
async function chatCompletionViaCustomOpenAICompatible(
  ...
  if (client.provider === "anthropic") {
    return chatCompletionViaCustomAnthropicCompatible(client, model, messages, resolved, onStreamProgress, onTextDelta, thinking, signal);
  }
  const baseUrl = client._piModel?.baseUrl ?? "";
  ...
  if (resolved.responseFormat) {
    payload.response_format = resolved.responseFormat.type === "json_schema"
      ? { type: "json_schema", json_schema: resolved.responseFormat.jsonSchema }
      : { type: "json_object" };
  }
  ...
  // 用于补发 thinking_start / thinking_end（上游 reasoning_content 不带边界事件）
  ...
  try {
    while (true) {
      ...
      for (const event of parsed.events) {
        ...
        if (delta) {
          ...
          if (reasoningDelta) {
            // 首次 reasoning 非空 delta → 补发 thinking_start
            if (!thinkingStarted) {
            ...
            thinking?.onThinkingDelta?.(reasoningDelta);
          }
        }
        if (json?.usage) {
          usage = {
    ...
    if (thinkingStarted) thinking?.onThinkingEnd?.();
    monitor.stop();
  }

  // 流结束仍缓冲在剥离器里的文本（未闭合的 think 块等）原样并回，避免数据丢失。
  content += thinkStripper.flush();
  // reasoningContent：有 thinking 回调的调用方已经流式收到了，但为了向后兼容
  // （不使用回调的调用方仍然要拿到 reasoning 结果），仍然用 content || reasoning
  // 作为兜底：只在 content 为空时使用。这样可以避免 reasoning 已经流式展示之后，
  // 末尾又再被并回 content 造成"思考内容又跑到正文里"的横跳。
...
export type ResponseFormat =
  ...
  | { readonly type: "json_schema"; readonly jsonSchema: unknown };
...
export async function chatCompletion(
  ...
  options?: {
    ...
    readonly thinking?: OnThinkingCallbacks;
    ...
    /**
     * Structured output format. When set, the OpenAI-compatible payload includes
     * `response_format`. For pi-ai path (non-custom transport), this is ignored
     * — the prompt-level JSON instruction is the primary mechanism there.
     */
    ...
    return await withTransientLLMRetry(
      async () => {
        ...
        if (shouldUseNativeCustomTransport(client)) {
          return chatCompletionViaCustomOpenAICompatible(client, model, messages, resolved, onStreamProgress, onTextDelta, thinking, signal);
        }
        return chatCompletionViaPiAi(client, model, messages, resolved, onStreamProgress, onTextDelta, thinking, signal);
      },
