...
export async function resolveEffectiveLLMConfig(
  ...
  const stubEnabled = isLlmStubEnabled();
  ...
  if (input.requireApiKey === false || stubEnabled) {
  ...
  if (!apiKey && input.requireApiKey !== false && !stubEnabled && !isApiKeyOptionalForEndpoint({ provider, baseUrl })) {
    throw new Error(
