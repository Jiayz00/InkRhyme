...
import { cachedReadFile } from "./book-context-cache.js";
import {
  filterActiveHooks,
  isFuturePlannedHook,
...
export async function retrieveMemorySelection(params: {
  ...
  const [
    ...
    cachedReadFile(join(storyDir, "pending_hooks.md")).catch(() => ""),
    cachedReadFile(join(storyDir, "volume_summaries.md")).catch(() => ""),
    ...
    try {
      if (memoryDb.getChapterCount() === 0) {
        const summaries = structuredSummaries?.rows ?? parseChapterSummariesMarkdown(
          await cachedReadFile(join(storyDir, "chapter_summaries.md")).catch(() => ""),
        );
  ...
  const [summariesMarkdown] = await Promise.all([
    cachedReadFile(join(storyDir, "chapter_summaries.md")).catch(() => ""),
  ]);
...
async function readStructuredState<T>(
  ...
  try {
    const raw = await cachedReadFile(path);
