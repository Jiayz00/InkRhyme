import { ChapterMemoSchema, PlannerMemoJsonSchema, type ChapterMemo, type PlannerMemoJson } from "../models/input-governance.js";
...
// === JSON-mode section headings ===
// Used to reconstruct the markdown body from structured JSON fields.
// Keys correspond to PlannerMemoJsonSchema field names.
interface MemoSectionHeading {
  readonly zh: string;
  readonly en: string;
}

const SECTION_HEADINGS: Record<keyof Omit<PlannerMemoJson, "goal" | "threadRefs">, MemoSectionHeading> = {
  currentTask: { zh: "## 当前任务", en: "## Current task" },
  readerExpectation: { zh: "## 读者此刻在等什么", en: "## What the reader is waiting for right now" },
  payoff: { zh: "## 该兑现的 / 暂不掀的", en: "## To pay off / to keep buried" },
  transitionalBeats: { zh: "## 日常/过渡承担什么任务", en: "## What the slow / transitional beats carry" },
  keyChoice: { zh: "## 关键抉择过三连问", en: "## Three-question check on the key choice" },
  endChange: { zh: "## 章尾必须发生的改变", en: "## Required end-of-chapter change" },
  hookLedger: { zh: "## 本章 hook 账", en: "## Hook ledger for this chapter" },
  doNot: { zh: "## 不要做", en: "## Do not" },
};

const SECTION_ORDER: ReadonlyArray<keyof typeof SECTION_HEADINGS> = [
  "currentTask",
  "readerExpectation",
  "payoff",
  "transitionalBeats",
...
function buildMemoBodyFromJson(json: PlannerMemoJson, language: "zh" | "en"): string {
  const headingKey = language === "en" ? "en" : "zh";
  return SECTION_ORDER
    .map((field) => `${SECTION_HEADINGS[field][headingKey]}\n${json[field]}`)
    .join("\n\n");
}

/**
 * Extract a JSON object from a raw LLM response that may include code fences
 * or leading/trailing prose. Returns the raw JSON string or throws if no
...
function extractJsonString(raw: string): string {
  ...
  // If the entire response is already a JSON object (starts with {)
  if (trimmed.startsWith("{")) return trimmed;
  // Find the first { ... } block (handles leading prose like "Here is the memo:\n{...}")
  const start = trimmed.indexOf("{");
  if (start < 0) throw new PlannerParseError("no JSON object found in response");
  // Brace-matching to find the closing }
  let depth = 0;
  let inString = false;
  let escape = false;
  ...
  throw new PlannerParseError("unterminated JSON object in response");
}

/**
 * Parse a planner memo produced by the LLM in JSON mode.
 *
 * Expects a JSON object matching PlannerMemoJsonSchema. Reconstructs the
 * markdown body (with all section headings) for downstream consumers that
 * expect the same body format as the legacy markdown parser.
 */
export function parseMemoJson(
  raw: string,
  ...
  try {
    ...
    throw new PlannerParseError(`JSON parse failed: ${error instanceof Error ? error.message : String(error)}`);
  }

  const json = PlannerMemoJsonSchema.parse(parsed);
  const body = buildMemoBodyFromJson(json, language);
