...
/**
 * 从 SkillResolutionResult 生成可直接拼接到 system prompt 的指导文本。
 * 用于 PipelineRunner 路径（自动化写作），只取强制 skill 的 body，
 * 不含意图选择 catalog（自动化路径不挂 use_skill 工具）。
 */
export function buildSkillGuidanceText(skills: SkillResolutionResult, isZh: boolean): string {
  const skillLines = skills.usedSkills.flatMap((skill) => {
    const line = `- ${skill.id} (${isZh ? "强制" : "forced"}): ${skill.description}`;
    const body = skill.body.trim();
    if (!body) return [line];
  ...
  const unavailable = skills.missingSkillIds.length > 0
    ...
    : "";
  return [
    isZh ? "## Skill 指导" : "## Skill Guidance",
    "",
    isZh
      ? "强制 Skill 是用户/界面明确要求的专业能力，除非不可用或违反安全/权限边界，否则必须按它的领域规则组织回答和工具提案。"
      : "Forced skills were explicitly requested; follow their domain guidance unless unavailable or unsafe.",
    ...skillLines,
    unavailable.trim(),
  ].filter(Boolean).join("\n");
}
