...
describe("PipelineRunner.writeChapters", () => {
  ...
  it("acquires the book lock per chapter while writing sequential chapters", async () => {
    ...
    expect(acquireBookLock).toHaveBeenCalledTimes(3);
  ...
  it("stops the batch after a state-degraded chapter", async () => {
    const root = await mkdtemp(join(tmpdir(), "inkos-batch-"));
    roots.push(root);
    const runner = new PipelineRunner({
      client: {} as never,
    ...
    const release = vi.fn(async () => undefined);
    const writeLocked = vi.fn()
      .mockResolvedValueOnce(chapter(8, "state-degraded"))
    ...
    expect(results[0]?.status).toBe("state-degraded");
