...
describe("StateValidatorAgent", () => {
  ...
  it("accepts a valid JSON object even when the model appends markdown with extra braces", async () => {
    ...
    // Pass authorityContext to bypass fast-path and exercise the LLM path
    await expect(agent.validate(
      ...
      "new hooks",
      "en",
      { storyFrame: "frame", bookRules: "rules", chapterSummaries: "summaries" },
    )).resolves.toEqual({
      warnings: [],
      passed: true,
    });
  ...
  it("uses fast-path local validation and skips LLM when no authority context", async () => {
    const agent = new StateValidatorAgent({
      client: {
        provider: "openai",
        apiFormat: "chat",
    ...
    // Simple state change with no hooks — fastValidate returns clean,
  ...
  it("throws when the validator model returns an empty response", async () => {
    ...
    // Empty response throws (fail-closed). Pass authorityContext to bypass
    // fast-path and ensure the LLM is actually called.
    await expect(agent.validate(
      "Chapter body.",
      3,
      "old state",
