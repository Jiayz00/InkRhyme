...
const VALID_MEMO_JSON = JSON.stringify({
  goal: "把七号门被动过手脚钉成现场实证",
  threadRefs: ["H03", "S004"],
  currentTask: "主角进入七号门现场，比对锁芯刮痕与监控时间线，把\"被动过手脚\"从猜测钉成实证。",
  readerExpectation: "1) 读者在等七号门是否有异常实锤\n2) 本章完全兑现，钉成现场实证",
  payoff: "- 该兑现：七号门异常 → 钉成现场实证\n- 暂不掀：幕后主使 → 压到第 20 章",
  transitionalBeats: "不适用 - 本章为高压实证章，无日常过渡段。",
  keyChoice: "- 主角本章最关键的一次选择：\n  - 为什么这么做？线索只剩这一条\n  - 符合当前利益吗？符合\n  - 符合他的人设吗？符合\n- 对手/配角本章最关键的一次选择：\n  - 为什么这么做？掩盖踪迹\n  - 符合当前利益吗？符合\n  - 符合他的人设吗？符合",
  endChange: "- 信息改变：主角掌握实证，可以面对幕后主使前先压住对手的退路",
  hookLedger: "advance:\n- H03 \"七号门异常\" → 从 pressured → near_payoff（本章钉成实证）\nresolve:\n- S004 \"锁芯刮痕\" → 核验完毕，本章结清\ndefer:\n- H07 \"幕后主使\" → 第 20 章再动",
  doNot: "- 不要让对手突然降智\n- 不要直接点破幕后主使",
});

function validMemoJson(chapter: number): string {
  // chapter is ignored in the JSON body — it comes from the host.
  return VALID_MEMO_JSON;
}

const VALID_MEMO_JSON_EN = JSON.stringify({
  goal: "Pin Door 7 tampering as live evidence",
  threadRefs: ["H03"],
  currentTask: "Pin the Door 7 tampering from suspicion to live evidence.",
  readerExpectation: "1) Reader expects to learn whether Door 7 is really compromised.\n2) This chapter pays it off in full — live evidence on stage.",
  payoff: "- Pay off: Door 7 anomaly → live evidence\n- Keep buried: the mastermind → push to chapter 20",
  transitionalBeats: "n/a — pressure chapter, no transitional beats.",
  keyChoice: "- Protagonist's most important choice this chapter:\n  - Why this choice? It is the only remaining lead.\n  - Does it match current interest? Yes.\n  - Does it match their persona? Yes.\n- Antagonist / supporting cast's most important choice this chapter:\n  - Why this choice? To cover their tracks.\n  - Does it match current interest? Yes.\n  - Does it match their persona? Yes.",
  endChange: "- Information change: protagonist holds live evidence.",
  hookLedger: "advance:\n- H03 \"Door 7 anomaly\" → pressured → near_payoff (pinned as live evidence this chapter)\ndefer:\n- H07 \"the mastermind\" → hold until chapter 20",
  doNot: "- Do not let the antagonist suddenly turn dumb.\n- Do not directly name the mastermind.",
});
...
describe("PlannerAgent.planChapter memo generation", () => {
  ...
  it("produces a valid ChapterMemo when the LLM returns well-formed JSON", async () => {
    const chatSpy = vi.spyOn(llmProvider, "chatCompletion").mockResolvedValue({
      content: validMemoJson(1),
      usage: ZERO_USAGE,
  ...
  it("does not hard-cap memo generation below the configured model output budget", async () => {
    ...
    const options = callArgs[3] as { temperature?: number; maxTokens?: number; responseFormat?: unknown } | undefined;
  ...
  it("calls the LLM exactly once with JSON responseFormat (no retry loop)", async () => {
    const chatSpy = vi.spyOn(llmProvider, "chatCompletion").mockResolvedValue({
      content: validMemoJson(4),
      usage: ZERO_USAGE,
    } as unknown as Awaited<ReturnType<typeof llmProvider.chatCompletion>>);
    ...
    // Verify responseFormat is set on the call
    const callArgs = chatSpy.mock.calls[0]!;
    const options = callArgs[3] as { responseFormat?: { type: string } } | undefined;
  ...
  it("returns a degraded memo instead of throwing when JSON parsing fails", async () => {
    const chatSpy = vi.spyOn(llmProvider, "chatCompletion").mockResolvedValue({
      content: "permanently broken — not JSON",
      usage: ZERO_USAGE,
    } as unknown as Awaited<ReturnType<typeof llmProvider.chatCompletion>>);
