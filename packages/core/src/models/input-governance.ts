...
/**
 * JSON schema for the planner LLM's structured output.
 *
 * Each field maps to a former Markdown section. The planner reconstructs the
 * markdown body from these fields for downstream consumers (composer, narrative
 * control, intent markdown). Using JSON eliminates Markdown section-parsing
 * failures and removes the need for retry loops.
 */
export const PlannerMemoJsonSchema = z.object({
  goal: z.string().min(1).max(50),
  threadRefs: z.array(z.string()).default([]),
  currentTask: z.string().min(1),
  readerExpectation: z.string().min(1),
  payoff: z.string().min(1),
  transitionalBeats: z.string().min(1),
  keyChoice: z.string().min(1),
  endChange: z.string().min(1),
  hookLedger: z.string().min(1),
  doNot: z.string().min(1),
});

export type PlannerMemoJson = z.infer<typeof PlannerMemoJsonSchema>;
