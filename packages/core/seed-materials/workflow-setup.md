...
> 来源：oh-story-claudecode `skills/story-long-write/references\workflow-setup.md`（MIT）
...
1. `ls 拆文库/`（数据源）与项目 `对标/`（引用视图）列已有书目；先按当前项目目录名、`.active-book` 和 `设定/题材定位.md` 中的本书信息识别当前作品，排除同名或来源指向当前正文的 `拆文库/{当前书}/`。story-import 为重建工程候选，也不得复制回 `对标/`。排除后都为空 → 跳到第 4 条。
2. 逐本读题材（短篇读 `拆文库/{书}/_meta.json` 的 `genre_detected`；长篇读 `概要.md` 头部或 `拆文报告.md`「基本信息」的「题材」），与本书题材/方向比对，标 同题材 / 弱相关 / 不相关。
3. 有同题材或弱相关候选 → 用 AskUserQuestion 推荐（列候选书 +「都不用」+「另拆一本」），主对标 1 本走文风/正文、其余作副对标。选定后登记」的 `主对标书` / `对标书列表`（Phase 2 落文件，此处先记选择），并按 SKILL.md「路径与术语约定」的「首次引用对标书」规则把选中书从 `拆文库/{书}/` 复制到 `对标/{书}/`。
4. 无候选 → 无对标继续（大纲按八节点自排，见 Phase 3）；用户想对标可先 `` 拆一本。
...
3. 任一主产物缺失时必须停止，设置 `missing_primary_contract: true`，并给出 `repair_action`：重跑 `` Stage 3+ 或重新 ``；不得以 `拆文报告.md`、章节摘要或故事线代替。
...
story-architect 属于高层级结构设计 agent。轻量题材定位优先由主会话完成；只有涉及复杂世界观、多线结构、强反转工程或用户明确要求时，才调用 story-architect。确认选题方向后，如果项目已部署 story-architect agent（检查 `.md` 是否存在），可 spawn `Agent(subagent_type: "story-architect", prompt: "项目目录：{dir}\n任务类型：题材定位\n查询参数：{用户选择的方向+对标信息}")` 辅助题材分析和核心梗设计。如 agent 不可用，由主线程直接执行。
...
核心设定阶段，如果项目已部署对应 agent（优先检查 `/` 下的 `story-architect.md` 和 `character-designer.md` 是否存在；不存在时再检查 `.opencode/agents/`，再不存在时检查 `agents/`），可 spawn 以下 agent 辅助：
...
- **追踪事务初始状态**：按 `references/tracking-transaction.md` 构造第 0 章初始化 JSON，执行 `scripts/ init`，生成检查点、续写状态卡（固定 7 栏）、空的伏笔/时间线视图与目录；核心角色动态快照可在第一次正文事务中创建
...
若已部署 story-architect agent（优先检查 `.md`），可让它辅助：
