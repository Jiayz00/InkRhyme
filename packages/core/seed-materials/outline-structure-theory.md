# outline-structure-theory

> 来源：oh-story-claudecode `skills/story-long-write/references\outline-structure-theory.md`（MIT）
...
5. **排章 + 落卷纲**：把换好（并修正）的关键情节落到本卷相同相对位置——这里的 1/4·中点·3/4 是**卷尺度**（1/4≈总章×0.25、中点×0.5、3/4×0.75）；卷里有几条一级结构时，每条一级结构内部另有自己的铺垫→逆转→收尾，别和卷尺度的关键点混为一谈。关键点之间用普通推进章过渡，每个关键点之后按需插一章呼吸 / 关系章；写进卷纲「对标结构坐标」与情绪弧线的章节定位列，细纲承接时每章标章节定位。同时把选中的剧情单元以 {书名}《剧情标题》剧情参照」。
