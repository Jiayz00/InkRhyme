...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "@inkrhyme/core", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
      ...
      expect(cliPackageJson.dependencies["@inkrhyme/core"]).toBe("0.4.8-canary.7");
    } finally {
  ...
  it("keeps source publish dependencies registry-installable", async () => {
    ...
    expect(cliPackageJson.dependencies["@inkrhyme/core"]).not.toMatch(/^workspace:/);
    expect(cliPackageJson.dependencies["@actalk/inkos-studio"]).not.toMatch(/^workspace:/);
    expect(studioPackageJson.dependencies["@inkrhyme/core"]).not.toMatch(/^workspace:/);
  });
  ...
  it("rejects workspace protocol in source publish manifests", async () => {
    ...
    try {
      ...
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@inkrhyme/core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
  ...
  it("replaces workspace dependencies before npm pack", { timeout: CLI_PACK_TEST_TIMEOUT_MS }, async () => {
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["@inkrhyme/core"]).toBe(corePackageJson.version);
